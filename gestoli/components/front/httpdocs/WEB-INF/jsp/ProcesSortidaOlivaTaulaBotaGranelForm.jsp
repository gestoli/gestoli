...
<html>
	<head>
		<title><fmt:message key="proces.sortidaOlivaBotaGranel.titol" /></title>
...
<%		} %>
	...
	<body>
		<form id="formulario" name="procesSortidaOlivaTaulaBotaGranelForm" action="ProcesSortidaOlivaTaulaBotaGranelForm.html?accio=${venda}" method="post" class="extended seguit">
			...
			<c:if test="${tipusSortida == 'b'}">
	
				<c:import url="comu/CampFormulari.jsp">
					<c:param name="tipus" value="hidden" />
					<c:param name="path" value="formData.idBota" />
					<c:param name="camp" value="idBota" />
					...
					<div class="cajaTabla">
						<div class="layoutSombraTabla">
							...
							<table cellpadding="0" cellspacing="0" class="listadoEstrecho noRoll">
								<thead>
									<tr>
										<th colspan="2" class="unicoHead">Bota: <c:out value=" ${bota.identificador}" /></th>
									</tr>
								</thead>
								<tbody>
									<tr>
										<td><fmt:message key="proces.envasatOliva.camp.kgRestants" /></td>
										<td class="boldtd"><c:out value="${bota.kgOlivaRestant}" /></td>
									</tr>
								</tbody>
							</table>
						</div>
...
<%-- 				</c:forEach> --%>
		...
		<form id="tornarForm" action="ProcesInici.html" class="seguit">
			<input type="hidden" id="tipusProces" name="tipusProces" value="25" />
