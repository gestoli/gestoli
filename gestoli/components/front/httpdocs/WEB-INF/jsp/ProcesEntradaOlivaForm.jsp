		...
		<script type="text/javascript" language="javascript">
			...
			<c:if test="${not empty appletRecOli}">
				function irEntradaOlivicultor() {
					...
					if (ent.value != ""){
						document.location = "ProcesEntradaOlivaForm.html?tipusProces=0&nif=" + ent.value;
					}
				}
			</c:if>
	...
	<body>
		<c:if test="${empty appletRecOli}">
			<div style="width: 495px; margin: auto;">
			    <p><strong><fmt:message key="consulta.entradaOliva.camp.olivicultor" />:</strong> <c:out value="${olivicultor.nom}" /> (RA-<c:out value="${olivicultor.codiDO}" /> RE-<c:out value="${olivicultor.codiDOExperimental}" />)</p>
			    <p><strong><fmt:message key="proces.entradaOliva.camp.produccio.restant"/>:</strong> <c:out value="${totalProduccioRestant}" /></p>
			</div>
			<form id="formulario" name="procesEntradaOlivaForm" action="ProcesEntradaOlivaForm.html" method="post" class="extended seguit">
				...
				<c:import url="comu/CampFormulari.jsp">
					<c:param name="tipus" value="hidden" />
					<c:param name="path" value="formData.mezclaValida" />
					<c:param name="name" value="mezclaValida" />
					<c:param name="camp" value="campo_mezclaValida" />
				</c:import>
				<c:import url="comu/CampFormulari.jsp">
				...
				<c:import url="comu/CampFormulari.jsp">
					...
					<c:param name="title">
						<fmt:message key="proces.entradaOlivaTaula.camp.esEcologic" />
					</c:param>
				...
				<c:import url="comu/CampFormulari.jsp">
					...
					<c:param name="path" value="formData.estatOliva" />
					<c:param name="required" value="required" />
					<c:param name="title"><fmt:message key="proces.entradaOliva.camp.estado" /></c:param>
					<c:param name="camp" value="campo_estatOliva" />
					<c:param name="name" value="estatOliva" />
					<c:param name="selectItems" value="estadosOliva" />
					<c:param name="selectItemsId" value="id" />
					<c:param name="selectItemsValue" value="nom" />
					<c:param name="selectSelectedValue" value="${formData.estatOliva}" />
...
%>
				...
				<div class="etiqueta <c:out value="${param.required}"/><c:if test="${not empty status.errorMessage}"> error</c:if>" style="display: none;">
					<c:import url="comu/CampFormulari.jsp">
						<c:param name="tipus" value="checkboxName" />
						<c:param name="path" value="formData.mezcla" />
						<c:param name="title"><fmt:message key="proces.entradaOliva.camp.mezcla" /></c:param>
						<c:param name="camp" value="campo_mezcla" />
						<c:param name="name" value="mezcla" />
						<c:param name="onclick" value="calculaPercentatgeMescla()" />
					</c:import>
				</div>
				<div class="separadorH"></div>
	...
	<c:if test="${not empty appletRecOli}">
		<div id="apprfid">
			...
			<form id="formulario" name="procesEntradaOlivaForm" action="ProcesEntradaOlivaForm.html" method="post" class="extended seguit" onsubmit="irEntradaOlivicultor(); return false;">
				<div class="separadorH"></div>
					<c:import url="comu/CampFormulari.jsp">
						<c:param name="tipus" value="hidden" />
						<c:param name="path" value="formData.olivicultorId" />
