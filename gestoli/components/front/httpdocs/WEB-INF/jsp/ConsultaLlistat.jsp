...
<html>
<head>
<title><fmt:message key="consulta.entradaOliva.titol" /></title>
...
<body>
<form id="formulario" action="ConsultaLlistat.html" method="post" class="extended seguit">
	<fieldset>
...
<c:if test="${llistatVarietats != null && not empty llistatVarietats}">
	...
	<div class="lotes" style="width: 745px; margin: 30px 0pt 0pt 40px;">
		<h3><fmt:message key="consulta.entradaOliva.titol.llistatVarietats" /></h3>
	</div>
...
<c:if test="${llistatVarietatsExperimentals != null && not empty llistatVarietatsExperimentals}">
	<div id="llistatVarietatsExperimentals"><%-- Tabla de resultados --%>
	<div class="lotes" style="width: 745px; margin: 30px 0pt 0pt 40px;">
		<h3><fmt:message key="consulta.entradaOliva.titol.llistatVarietatsExperimentals" /></h3>
	</div>
	<div class="layoutSombraTabla ancho">
	<div class="rellenoInf"></div>
	<div class="rellenoIzq"></div>
...
<jsp:scriptlet>
       ...
       subtotals.setGrandTotalDescription("Total");    // optional, defaults to Grand Total
       subtotals.setSubtotalLabel("{0} Subtotal", request.getLocale());      // optional, defaults to "{0} Total"
       pageContext.setAttribute("subtotalerVarietatsExperimentals", subtotals);
    </jsp:scriptlet>
    
	<display:table name="llistatVarietatsExperimentals" requestURI="" id="llistatVarietatsExperimentals" export="true" sort="list" cellpadding="0" cellspacing="0" class="listadoAncho selectable noEnlace" decorator="subtotalerVarietatsExperimentals"  defaultsort="1">
		<display:column titleKey="consulta.entradaOliva.camp.nom" sortable="true" property="[1]" sortProperty="[1]"  />
		<display:column titleKey="consulta.entradaOliva.camp.quilosTotals" sortable="true" property="[2]" sortProperty="[2]" total="true" format="{0,number,#,##0.00} kg." />
		<display:setProperty name="export.xml" value="false" />
   		<display:setProperty name="export.csv" value="false" />
   		...
   		<display:setProperty name="export.pdf" value="false" />
   		<display:setProperty name="export.excel.include_header" value="true" />
   		<display:setProperty name="export.excel.filename" value="OlivaVarietatsExperimentals.xls" />
   		<display:setProperty name="export.decorated" value="true" />
	</display:table>
...
<jsp:scriptlet>
	...
	<display:table name="llistat" requestURI="" id="partidaOliva"  export="true" sort="list" cellpadding="0" cellspacing="0" class="listadoAncho selectable noEnlace" decorator="subtotaler">
		...
		<display:column titleKey="consulta.entradaOliva.camp.numeroEntrada" sortable="true" style="${color}">
			<c:out value="${partidaOliva.numeroEntrada}" />
		</display:column>
		...
		<display:column titleKey="consulta.entradaOliva.camp.experimental" headerClass="final" sortable="true" style="${color}">
			<c:choose>
				<c:when test="${partidaOliva.teVarietatsExperimental}"><fmt:message key="manteniment.si" /></c:when>
