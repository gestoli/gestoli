...
<%		} %>
	...
	<body>
		<form id="formulario" name="procesSortidaOlivaTaulaForm" action="ProcesSortidaOlivaTaulaForm.html?accio=${venda}" method="post" class="extended seguit">
			...
			<c:if test="${tipusSortida == 'l'}">
				...
				<c:forEach var="lot" items="${lots}" varStatus="status">
					...
					<div class="separadorH"></div>
					<c:set var="oliva" value="${lot.olivaTaula}" />
				</c:forEach>
	
				<c:if test="${oliva == true}">
					<c:import url="comu/CampFormulari.jsp">
						<c:param name="tipus" value="checkbox" />
						<c:param name="path" value="formData.desqualificar" />
						<c:param name="title"><fmt:message key="sortidaLot.camp.oliva.desqualificar" /></c:param>
		...
		<form id="tornarForm" action="ProcesInici.html" class="seguit">
			<input type="hidden" id="tipusProces" name="tipusProces" value="21" />
