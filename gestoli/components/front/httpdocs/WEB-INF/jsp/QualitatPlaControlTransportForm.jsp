...
<form id="formulario" name="PlaControlTransportForm" action="QualitatPlaControlTransportForm.html" method="post" class="extended seguit" onsubmit="">
	...
	<c:import url="comu/CampFormulari.jsp">
		<c:param name="tipus" value="hidden" />
		<c:param name="path" value="formData.establimentId" />
		...
		<c:import url="comu/CampFormulari.jsp">
			...
			<c:param name="path" value="formData.vehicleId" />
			<c:param name="title">
				<fmt:message key="qualitat.plaControlTransport.camp.vehicle" />
			</c:param>
			<c:param name="camp" value="campo_vehicleId" />
			<c:param name="name" value="vehicleId" />
			<c:param name="required" value="required" />
			<c:param name="selectItems" value="equips" />
			<c:param name="selectItemsId" value="id" />
			<c:param name="selectItemsValue" value="nom" />
			<c:param name="selectSelectedValue" value="${formData.vehicleId}" />
	...
	<c:import url="comu/CampFormulari.jsp">
		...
		<c:param name="title">
			<fmt:message key="qualitat.plaControlTransport.camp.dataVerificacio" />
		</c:param>
	...
	<c:import url="comu/CampFormulari.jsp">
		<c:param name="tipus" value="select" />
		<c:param name="path" value="formData.responsableVerificacioId" />
		<c:param name="title">
			<fmt:message key="qualitat.plaControlTransport.camp.responsable" />
		</c:param>
		<c:param name="camp" value="campo_idRresponsableVerificacio" />
		<c:param name="name" value="responsableVerificacioId" />
		<c:param name="required" value="required" />
		<c:param name="selectItems" value="personal" />
		<c:param name="selectItemsId" value="id" />
		<c:param name="selectItemsValue" value="nom" />
		<c:param name="selectSelectedValue" value="${formData.responsableVerificacioId}" />
		<c:param name="clase" value="campoFormGrande conMargen" />
		<c:param name="disabled" value="${disabled}" />
	</c:import>
	...
	<div class="off" id="estibaCorrectaX">
		<div class="etiqueta conMargen <c:if test="${not empty status.errorMessage}"> error</c:if>">
			<c:import url="comu/CampFormulari.jsp">
				<c:param name="tipus" value="checkbox" />
				<c:param name="path" value="formData.estibaCorrecta" />
				<c:param name="title">
					<fmt:message key="qualitat.plaControlTransport.camp.estibaCorrecta" />
				</c:param>
				<c:param name="camp" value="estibaCorrecta" />
				<c:param name="name" value="estibaCorrecta" />
				<c:param name="required" value="required" />
	...
	<div class="off" id="netejaCorrectaX">
		<div class="etiqueta conMargen <c:if test="${not empty status.errorMessage}"> error</c:if>">
			<c:import url="comu/CampFormulari.jsp">
				<c:param name="tipus" value="checkbox" />
				<c:param name="path" value="formData.netejaCorrecta" />
				<c:param name="title">
					<fmt:message key="qualitat.plaControlTransport.camp.netejaCorrecta" />
				</c:param>
				<c:param name="camp" value="netejaCorrecta" />
				<c:param name="name" value="netejaCorrecta" />
				<c:param name="required" value="required" />
				<c:param name="onclick" value="modificarVisibilidadNoConformidad()" />
				<c:param name="disabled" value="${disabled}" />
			</c:import>
			...
			<div id="listado"><%-- Tabla de resultados --%>
				<div class="layoutSombraTabla ancho">
					...
					<display:table name="formData.noConformitats" id="registre" requestURI="" export="true" sort="list" cellpadding="0" cellspacing="0" class="listadoEstrecho" defaultsort="1" defaultorder="descending">
						...
						<display:setProperty name="export.excel.filename" value="qualitatPlaControlTransportNoConformitat.xls" />
	...
	<div class="botonesForm">
		...
		<div class="btnCorto" 
			...
			onclick="document.location ='QualitatPlaControlTransport.html';"><a
	...
	<form id="deleteForm" action="QualitatPlaControlTransport.html" method="post"
		class="seguit"
	...
	// <![CDATA[
		...
		function modificarVisibilidadNoConformidad() {
			var estibaCorrecta = document.getElementById("estibaCorrecta");
			var netejaCorrecta = document.getElementById("netejaCorrecta");
			if ((estibaCorrecta != null && estibaCorrecta.checked) && (netejaCorrecta != null && netejaCorrecta.checked)){
