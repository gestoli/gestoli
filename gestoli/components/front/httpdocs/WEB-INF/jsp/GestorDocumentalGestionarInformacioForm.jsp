...
<form id="formulario" name="gestionarInformacioForm"
	action="GestorDocumentalGestionarInformacioForm.html" method="post"
	...
	<c:param name="selectSelectedValue"
		value="${formData.gestorDocumentalCategoriaInformacio.id}" />
...
<c:import url="comu/CampFormulari.jsp">
	<c:param name="tipus" value="select" />
	<c:param name="path" value="formData.estat" />
	<c:param name="required" value="required" />
	<c:param name="title">
		<fmt:message key="informacio.camp.estat" />
	</c:param>
	<c:param name="camp" value="campo_estat" />
	<c:param name="name" value="estat" />
	<c:param name="selectItems" value="estats" />
	<c:param name="selectItemsId" value="id" />
	<c:param name="selectItemsValue" value="nom" />
	<c:param name="selectSelectedValue"
...
<c:if test="${formData.id != null && not empty formData.id}">
	<c:if test="${not empty llistatDocuments}">
		...
		<display:table name="llistatDocuments" requestURI="" id="document" export="true" sort="list" cellpadding="0" cellspacing="0"
			class="listadoEstrecho selectable">
			<display:column titleKey="document.camp.titol" headerClass=""
				sortable="true" sortProperty="titol">
				<a
					href="GestorDocumentalGestionarDocumentForm.html?id=<c:out value="${document.id}"/>"><c:out
...
<form id="tornarForm" action="GestorDocumentalGestionarInformacio.html" class="seguit">
</form>

<form name="formularioDocument" action="GestorDocumentalGestionarDocumentForm.html"
	class="seguit"><input id="idInformacio" name="idInformacio"
	value="<c:out value="${formData.id}"/>" type="hidden" /></form>

<form id="deleteForm" action="GestorDocumentalGestionarInformacio.html" method="post"
	class="seguit"
