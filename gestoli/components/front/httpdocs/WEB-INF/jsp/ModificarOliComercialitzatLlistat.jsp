...
<body>
<form id="formulario" action="ModificarOliComercialitzatLlistat.html" method="post" class="extended seguit">
		<fieldset>
			...
			<c:import url="comu/CampFormulari.jsp">
				...
				<c:param name="required" value="required" />
				<c:param name="title"><fmt:message key="diposit.camp.document" /></c:param>
...
<c:if test="${llistat != null && not empty llistat}">
	...
	<display:table name="llistat" requestURI="" id="sortidaLot" export = "true" sort="list" cellpadding="0" cellspacing="0" class="listadoAncho selectable noEnlace" decorator="subtotaler2" defaultsort="1">
		<display:column titleKey="consulta.oliComercialitzat.camp.document" headerClass="ancho160" sortable="true" group="1">
...
<%-- 		</display:column> --%>
		<display:column headerClass="ancho20 paddingCelda final">
			<a href="ModificarSortidaLotVenda.html?sortidaLotId=<c:out value="${sortidaLot.id}"/>&amp;tipusSortida=<c:out value="l"/>&amp;dataSortida=<c:out value="${sortidaLot.vendaDataFormat}"/>"><img	
				src="img/icons/edit.png" border="0"
