...
public class ModificarSortidaFormController extends SimpleFormController {

	private static Logger logger = Logger.getLogger(ModificarSortidaFormController.class);
	...
	throws ServletException {
		...
		ModificarSortidaCommand modificarSortidaCommand = (ModificarSortidaCommand)command;

		try {
			String tipus = modificarSortidaCommand.getTipusSortida();
			Long lid = modificarSortidaCommand.getIdSortida();
			...
			if (tipus.equals("diposit")) {
				...
				if (sortidaDiposit != null) {
					sortidaDiposit.setVendaTipusDocument(modificarSortidaCommand.getTipusDocument());
					sortidaDiposit.setVendaNumeroDocument(modificarSortidaCommand.getDocument());
					
					oliInfraestructuraEjb.setHibernateTemplate(getHibernateTemplate());
					oliInfraestructuraEjb.actualitzaSortidaDiposit(sortidaDiposit);
				}
			} else if (tipus.equals("l")) {
				oliInfraestructuraEjb.setHibernateTemplate(getHibernateTemplate());
				SortidaLot sortidaLot = oliInfraestructuraEjb.sortidaLotAmbId(lid);
				
				if (sortidaLot != null) {
					sortidaLot.setVendaTipusDocument(modificarSortidaCommand.getTipusDocument());
					sortidaLot.setVendaNumeroDocument(modificarSortidaCommand.getDocument());
	...
	throws Exception {
		...
		myModel.put("infoSortida", modificarSortidaCommand.getInfoSortida());
	...
	protected Object formBackingObject(HttpServletRequest request,
			@RequestParam("tipus") String tipus) throws ServletException {
		ModificarSortidaCommand modificarSortidaCommand = new ModificarSortidaCommand();
		...
		try {
			...
			if (!id.equals("")) {
				...
				modificarSortidaCommand.setTipusSortida(tipus);
				modificarSortidaCommand.setIdSortida(lid);
				...
				try {
					if (tipus.equals("diposit")) {
						...
						if (sortidaDiposit != null) {
							modificarSortidaCommand.setTipusDocument(sortidaDiposit.getVendaTipusDocument());
							modificarSortidaCommand.setDocument(sortidaDiposit.getVendaNumeroDocument());
						...
						if (sortidaLot != null) {
							modificarSortidaCommand.setTipusDocument(sortidaLot.getVendaTipusDocument());
							modificarSortidaCommand.setDocument(sortidaLot.getVendaNumeroDocument());
		...
		modificarSortidaCommand.setInfoSortida(infoSortida);
		
		return modificarSortidaCommand;
	}
