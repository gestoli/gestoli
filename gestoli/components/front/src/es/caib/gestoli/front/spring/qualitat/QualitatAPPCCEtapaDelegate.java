...
public class QualitatAPPCCEtapaDelegate implements ApplicationContextAware {
	
	private static Logger logger = Logger.getLogger(QualitatAPPCCEtapaDelegate.class);
    ...
    public ModelAndView list(
        ...
        try {
			...
			//ResourceBundle bundle = ResourceBundle.getBundle("messages",new java.util.Locale(lang));
	   		Collection llistat = oliQualitatEjb.aPPCCEtapaCercaTotsPerEstabliment(idEstablecimiento);
	   		/*for (Object etapa: llistat){
        		if ("".equals(((QualitatAPPCCEtapa)etapa).getPuntControl())){
        			((QualitatAPPCCEtapa)etapa).setPuntControl(bundle.getString("qualitat.appcc.control.pendent"));
        			((QualitatAPPCCEtapa)etapa).setPerillControlat("No");
        		}
	   		}*/
            ...
            logger.info("Obtenint llistat d'etapes d'APPCC: " + llistat.size() + " registres trobats");

        } catch (Exception ex) {
            logger.error("Error obtenint llistat d'etapes d'APPCC", ex);
            ControllerUtils.saveMessageError(request, missatge("qualitat.appcc.etapa.missatge.llistat.no", request));
        }
        ...
        for (int i=0; i<ids.length; i++) {
            ...
            if (id != null && !"".equals(id)) {
                try {
                	...
                	oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
                    oliQualitatEjb.aPPCCEtapaEsborrar(Long.valueOf(id));
