...
public class QualitatAiguaPuntSortidaFormController extends SimpleFormController{
...
private static Logger logger = Logger.getLogger(QualitatPlaFormacioFormController.class);
	...
	throws ServletException {
		...
		QualitatAiguaPuntSortidaCommand qualitatAiguaPuntSortidaCommand = (QualitatAiguaPuntSortidaCommand)command;
		Long sortidaId = qualitatAiguaPuntSortidaCommand.getId();
		...
		QualitatAiguaPuntSortida sortida = null;
		
		try{
			oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
			if (isCreate(request)){
				logger.info("Creant el registre: " + qualitatAiguaPuntSortidaCommand);
				sortida = new QualitatAiguaPuntSortida();
				sortida.setEstabliment(est);
			} else {
				logger.info("Actualitzant el registre: " + qualitatAiguaPuntSortidaCommand);
				sortida = oliQualitatEjb.aiguaPuntSortidaAmbId(sortidaId);
			}
			
			sortida.setUbicacio(qualitatAiguaPuntSortidaCommand.getUbicacio());
			
			
			// Creamos el plan de mantenimiento
			sortida.setId(oliQualitatEjb.crearAiguaPuntSortida(sortida));
	...
	protected Map referenceData(
		...
		myModel.put("path", "qualitat_PlaAbastamentAigua.puntsSortida");
	...
	protected Object formBackingObject(HttpServletRequest request) throws ServletException {
		try {
			QualitatAiguaPuntSortidaCommand command = new QualitatAiguaPuntSortidaCommand();
			...
			if ((!isFormSubmission(request)) && (!isCreate(request))) {
				try {
					...
					oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
					QualitatAiguaPuntSortida sortida = oliQualitatEjb.aiguaPuntSortidaAmbId(id);
					command.fromQualitatAiguaPuntSortida(sortida);
