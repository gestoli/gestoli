							...
							if (descomposicioPlantacio.getVarietatOliva().getId() == Constants.VARIETAT_OLIVA_MALLORQUINA) {
								...
								if(trTaula!=null){
									if (trTaula.equals("hectarea")){
										dpot.setMaxProduccio(rvTaula * dpot.getSuperficie());
									}else {
										dpot.setMaxProduccio(rvTaula * dpot.getNumeroOliveres());
									}
								}else{
									dpot.setMaxProduccio(new Double(0));
								}
