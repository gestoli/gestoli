...
public class QualitatAPPCCEtapaPerillDelegate implements ApplicationContextAware {
	private static Logger logger = Logger.getLogger(QualitatAPPCCEtapaPerillDelegate.class);
       ...
       for (int i=0; i<ids.length; i++) {
           ...
           if (id != null && !"".equals(id)) {
               try {
               	...
               	logger.info("Eliminant el APPCCEtapaPerill [" + id + "]");
               	oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
                   oliQualitatEjb.aPPCCEtapaPerillEsborrar(Long.valueOf(id));

               } catch (Exception ex) {
                   logger.error("Error esborrant el APPCCEtapaPerill [" + id + "]", ex);
                   exito = false;
               }
           }
       }

       String forward = deleteView + "?idEtapa=" + request.getParameter("idEtapa");
