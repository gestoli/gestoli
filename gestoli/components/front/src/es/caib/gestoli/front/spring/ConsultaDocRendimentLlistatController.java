...
import es.caib.gestoli.front.spring.ConsultaLlibresFormController.TipusMesura;
 ...
 *
 */
public class ConsultaDocRendimentLlistatController extends SimpleFormController {

    /*
     * Objeto para escribir mensajes de log.
     */
	private static Logger logger = Logger.getLogger(ConsultaDocRendimentLlistatController.class);
    ...
    throws ServletException {
    	ConsultaDocRendimentLlistatCommand consulta = (ConsultaDocRendimentLlistatCommand) command;
    	...
    	try {
    		...
    		myModel.put("path", "consultar_docRendiment");
    		if (consulta.getFromEstabliment() != null && consulta.getFromEstabliment().equalsIgnoreCase("true")){	
	    		...
	    		if(request.getParameter("fromEstabliment")!= null){
	    			...
	    			if(establiment.getNom()!= null && !establiment.getNom().equalsIgnoreCase("")){
	    				myModel.put("path", "consulta_docRendimentEstabliment");
        ...
        try {
        	...
        	if (Util.isDataCorrecta(request.getParameter("dataInici"), "dd/MM/yyyy")
        		...
        		for(Iterator it= listaElaboraciones.iterator(); it.hasNext();){
        			...
        			if( oliRetiratPropietarioElaboracio != null && oliRetiratPropietarioElaboracio.size() > 0){
        				Double[] litresRetirats = new Double[oliRetiratPropietarioElaboracio.size()];
        				Double[] kilosRetirats = new Double[oliRetiratPropietarioElaboracio.size()];
        				Long[] idOlivicultors = new Long[oliRetiratPropietarioElaboracio.size()];
        				int index = 0;
        				
        				for(Object o: oliRetiratPropietarioElaboracio){
        					EntradaDiposit ed = (EntradaDiposit)o;
        					litresRetirats[index] = ed.getLitres();
        					kilosRetirats[index] = ed.getLitres() * 0.916;
        					idOlivicultors[index] = (ed.getOlivicultor() != null)?ed.getOlivicultor().getId():null;
        			...
        			if( entradasDipositElaboracioHash.get(elaboracio.getId()) != null ){
        				...
        				if(dipositList.size()>0){
        					Diposit[] diposits = new Diposit[dipositList.size()];
        					dipositList.toArray(diposits);
        			...
        			procesElaboracioOliCommand.setRendiment(procesElaboracioOliCommand.rendiment());
        			
        			procesElaboracioOliCommand.setNumPrintsDocRendiment(elaboracio.getNumPrintsDocRendiment());
        	...
        	logger.error("Error obtenint llistat de documents de rendiment", ex);
        }
        return myModel;
    }
    ...
    protected Object formBackingObject(HttpServletRequest request) throws ServletException {
    	ConsultaDocRendimentLlistatCommand consulta = (ConsultaDocRendimentLlistatCommand)request.getSession().getAttribute("consulta");
    	if (consulta == null) 
    		consulta = new ConsultaDocRendimentLlistatCommand();
    	else 
