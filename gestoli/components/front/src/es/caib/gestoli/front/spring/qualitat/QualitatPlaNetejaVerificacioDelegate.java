...
public class QualitatPlaNetejaVerificacioDelegate  implements ApplicationContextAware {
	
	private static Logger logger = Logger.getLogger(QualitatPlaNetejaVerificacioDelegate.class);
    ...
    public ModelAndView list(
        ...
        try {
    		...
    		Establiment est = (Establiment)session.getAttribute(establimentSessionKey);
	   		...
	   		Long idNeteja = new Long(Long.parseLong(request.getParameter("idNeteja")));
	   		...
	   		Collection llistat = oliQualitatEjb.netejaVerificacioPerPlaNeteja(idNeteja);
        	for (Object net: llistat){
        		if (((Boolean)true).equals(((QualitatPlaNetejaVerificacio)net).getSatisfactori()))
        			((QualitatPlaNetejaVerificacio)net).setSatisfactoriString(bundle.getString("qualitat.plaNeteja.verificacio.satisfactoria.si"));
        		else ((QualitatPlaNetejaVerificacio)net).setSatisfactoriString(bundle.getString("qualitat.plaNeteja.verificacio.satisfactoria.no"));
        	}
            ...
            logger.info("Obtenint llistat de PlaNetejaVerificacio: " + llistat.size() + " registres trobats");

        } catch (Exception ex) {
            logger.error("Error obtenint llistat de PlaNetejaVerificacio", ex);
        ...
        for (int i=0; i<ids.length; i++) {
            ...
            if (id != null && !"".equals(id)) {
                try {
                	...
                	logger.info("Eliminant el PlaNetejaVerificacio [" + id + "]");
                	oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
                    oliQualitatEjb.netejaVerificacioEsborrar(Long.valueOf(id));

                } catch (Exception ex) {
                    logger.error("Error esborrant el PlaNetejaVerificacio [" + id + "]", ex);
                    exito = false;
                }
            }
        }

        String forward = deleteView + "?idNeteja=" + request.getParameter("idNeteja");
