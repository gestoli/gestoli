...
public class QuadernCampDelegate  implements ApplicationContextAware {
	
	private static Logger logger = Logger.getLogger(QuadernCampDelegate.class);
        ...
        for (int i=0; i<ids.length; i++) {
            ...
            if (id != null && !"".equals(id)) {
                try {
                	...
                	logger.info("Eliminant el quadern de camp [" + id + "]");
                	oliInfraestructuraEjb.setHibernateTemplate(getHibernateTemplate());
                    oliInfraestructuraEjb.quadernCampEsborrar(Long.valueOf(id));

                } catch (Exception ex) {
                    logger.error("Error esborrant el quadern de camp [" + id + "]", ex);
                    exito = false;
                }
            }
        }
