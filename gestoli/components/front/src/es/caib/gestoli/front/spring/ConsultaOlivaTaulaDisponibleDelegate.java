 ...
 */
public class ConsultaOlivaTaulaDisponibleDelegate implements ApplicationContextAware {
	private static Logger logger = Logger.getLogger(ConsultaOlivaTaulaDisponibleDelegate.class);
...
//        		llistatEstabliments = oliInfraestructuraEjb.establimentCercaTotsActivos(campanyaId);
        		llistatEstabliments = oliInfraestructuraEjb.establimentProductorOlivaCercaTotsActivos(campanyaId);
        	}
        	...
        	for(Object o: llistatEstabliments){
        		...
        		Double botaVerda = 0.0;
        		Double botaTrencada= 0.0;
        		Double botaNegra = 0.0;
        		
        		Integer potsVerda = 0;
			   	Integer potsTrencada= 0;
			   	Integer potsNegra = 0;
			   	
			   	Boolean olivaTaula = e.getOlivaTaula()==null ? false : true;
        		if (olivaTaula) {
		        	...
		        	// Cerca tots las diposits de un establecimiento no vacios y que pertenecen a unas determinadas categorias
				   	...
				   	Collection listaBotas = oliConsultaEjb.findBotasActivasByEstablecimientoAndData(e.getId(), new Date());
				   
				   	for(Object obj: listaBotas){
				   		Bota bota = (Bota)obj;
				   		
				   		if(bota.getTipusOlivaTaula()==Constants.TIPUS_OLIVA_TAULA_VERDA){
				   			botaVerda += bota.getKgOlivaRestant(); 
				   		} else if(bota.getTipusOlivaTaula()==Constants.TIPUS_OLIVA_TAULA_TRENCADA){ 
				   			botaTrencada += bota.getKgOlivaRestant(); 
				   		} else if(bota.getTipusOlivaTaula()==Constants.TIPUS_OLIVA_TAULA_NEGRA){ 
				   			botaNegra += bota.getKgOlivaRestant(); 
				   		}
				   	}
			        ...
			        oliConsultaEjb.setHibernateTemplate(getHibernateTemplate());
			        Collection listaLotes = oliConsultaEjb.findLotsOlivaTaulaAmbExistenciaByEstablecimientoAndData(e.getId(), new Date());
...
//			        	if(sortidalot.getLot()!=null) {
					   		if(lot.getTipusOlivaTaula()==Constants.TIPUS_OLIVA_TAULA_VERDA){
					   			potsVerda += lot.getNumeroBotellesActuals(); 
					   		} else if(lot.getTipusOlivaTaula()==Constants.TIPUS_OLIVA_TAULA_TRENCADA){ 
					   			potsTrencada += lot.getNumeroBotellesActuals(); 
					   		} else if(lot.getTipusOlivaTaula()==Constants.TIPUS_OLIVA_TAULA_NEGRA){ 
					   			potsNegra += lot.getNumeroBotellesActuals(); 
					   		}
//			        	}
				   	}
