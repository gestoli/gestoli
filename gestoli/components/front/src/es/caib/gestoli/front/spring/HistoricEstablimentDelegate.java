...
/**
 * Classe delegada per mostrar el llistat d'històrics d'establiments.
 * 
 * @author Miquel Angel Amengual <miquelaa@limit.es>
 */
public class HistoricEstablimentDelegate implements ApplicationContextAware {
	private static Logger logger = Logger.getLogger(HistoricEstablimentDelegate.class);
	...
	public ModelAndView list(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			if (request.isUserInRole(rolDoGestor)) {
				...
				Collection llistat = oliInfraestructuraEjb.historicsEstabliments();
				myModel.put("llistat", llistat);
				myModel.put("path", "llista_historic_establiment");
			}
		} catch (Exception ex) {
			logger.error("Error obtenint llistat d'històrics d'establiments", ex);
			ControllerUtils.saveMessageError(request, missatge("historic.establiment.missatge.llistat.no", request));
		}
