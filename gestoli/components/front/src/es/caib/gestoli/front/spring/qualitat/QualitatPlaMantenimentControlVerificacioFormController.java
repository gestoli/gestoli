...
public class QualitatPlaMantenimentControlVerificacioFormController  extends SimpleFormController {
	...
	throws ServletException {
		...
		try{
			...
			if (!isCreate(request)){
				logger.info("Verificant el registre: " + qualitatPlaMantenimentControlCommand);
				plaMantenimentControl = oliQualitatEjb.mantenimentControlAmbId(mantenimentControlId);
				
				plaMantenimentControl.setObjectiu(qualitatPlaMantenimentControlCommand.getObjectiu());
				plaMantenimentControl.setDataVerificacio(qualitatPlaMantenimentControlCommand.getDataVerificacio());
				
				if (qualitatPlaMantenimentControlCommand.getIdResponsableVerificacio() != null){
					QualitatDescripcioPersonal responsableVerificacio = oliQualitatEjb.personalAmbId(qualitatPlaMantenimentControlCommand.getIdResponsableVerificacio());
					plaMantenimentControl.setResponsableVerificacio(responsableVerificacio);
				}
				
				Boolean isSatisfactori = qualitatPlaMantenimentControlCommand.getSatisfactori();
				if (isSatisfactori == null)
					isSatisfactori = new Boolean(false);
				else
					isSatisfactori = new Boolean(true);
				plaMantenimentControl.setSatisfactori(isSatisfactori);
				
				// Actualizamos la verificación del control del plan de mantenimiento
				plaMantenimentControl.setId(oliQualitatEjb.crearPlaMantenimentControl(plaMantenimentControl));
				
				ControllerUtils.saveMessageInfo(request, missatge("qualitat.plaManteniment.control.verificacio.missatge.modificar.ok"));
				
			}
		}catch (Exception ex) {
			logger.error("Error creant la verificacio del control del pla de manteniment", ex);
			ControllerUtils.saveMessageError(request, missatge("qualitat.plaManteniment.control.verificacio.missatge.modificar.no"));
		}
	...
	protected Map referenceData(
		...
		myModel.put("path", "qualitat_PlaManteniment.control.verificacio");
       ...
       /* if (queryString != null) {
        ...
        myModel.put("url", "plaManteniment");
