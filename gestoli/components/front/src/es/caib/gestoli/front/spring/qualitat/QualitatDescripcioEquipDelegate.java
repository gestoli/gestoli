...
public class QualitatDescripcioEquipDelegate implements ApplicationContextAware {
	
	private static Logger logger = Logger.getLogger(QualitatDescripcioPersonalDelegate.class);
    ...
    public ModelAndView list(
        ...
        try {
        	//if (request.isUserInRole(rolDoGestor)) {
	        	...
	        	Collection llistat = oliQualitatEjb.getQualitatDescripcioEquipsByEstabliment(idEstablecimiento);
	        	oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
	        /*	if (oliQualitatEjb.existenZonasActiusEnEstabliment(idEstablecimiento).booleanValue()){
	            ...
	            logger.info("Obtenint llistat de equips: " + llistat.size() + " registres trobats");

        	//}
	         
        } catch (Exception ex) {
            logger.error("Error obtenint llistat d'equips", ex);
            ControllerUtils.saveMessageError(request, missatge("qualitat.descripcio.equip.missatge.llistat.no", request));
        }
        ...
        for (int i=0; i<ids.length; i++) {
            ...
            if (id != null && !"".equals(id)) {
                try {
                	...
                	logger.info("Eliminant l'equip [" + id + "]");
                	oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
                    oliQualitatEjb.qualitatDescripcioEquipEsborrar(Long.valueOf(id));

                } catch (Exception ex) {
                    logger.error("Error esborrant l'equip [" + id + "]", ex);
                    exito = false;
                }
            }
        }
