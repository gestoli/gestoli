...
public class QualitatControlPlaguesVerificacioDelegate  implements ApplicationContextAware {
	private static Logger logger = Logger.getLogger(QualitatControlPlaguesVerificacioDelegate.class);
       ...
       try {
		...
		ResourceBundle bundle = ResourceBundle.getBundle("messages",new java.util.Locale(lang));
       	...
       	for (Object plag : llistat){
       		if ("roe".equals(((QualitatControlPlaguesVerificacio)plag).getObjectiu()))
       			((QualitatControlPlaguesVerificacio)plag).setObjectiu(bundle.getString("qualitat.controlPlagues.plagues.roedors"));
       		else if ("nov".equals(((QualitatControlPlaguesVerificacio)plag).getObjectiu()))
       			((QualitatControlPlaguesVerificacio)plag).setObjectiu(bundle.getString("qualitat.controlPlagues.plagues.noVoladors"));
       		else if ("siv".equals(((QualitatControlPlaguesVerificacio)plag).getObjectiu()))
       			((QualitatControlPlaguesVerificacio)plag).setObjectiu(bundle.getString("qualitat.controlPlagues.plagues.siVoladors"));
       	}
           ...
           logger.info("Obtenint llistat de ControlPlaguesVerificacio: " + llistat.size() + " registres trobats");

       } catch (Exception ex) {
           logger.error("Error obtenint llistat de ControlPlaguesVerificacio", ex);
        ...
        for (int i=0; i<ids.length; i++) {
            ...
            if (id != null && !"".equals(id)) {
                try {
                	...
                	logger.info("Eliminant el ControlPlaguesVerificacio [" + id + "]");
                	oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
                    oliQualitatEjb.controlPlaguesVerificacioEsborrar(Long.valueOf(id));

                } catch (Exception ex) {
                    logger.error("Error esborrant el ControlPlaguesVerificacio [" + id + "]", ex);
                    exito = false;
                }
            }
        }
