...
public class QualitatDescripcioPersonalDelegate implements ApplicationContextAware {
    ...
    public ModelAndView list(
        ...
        try {
        	//if (request.isUserInRole(rolDoGestor)) {
	        	...
	        	Collection llistat = oliQualitatEjb.personalCercaTotsPerEstabliment(idEstablecimiento);
	        	oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
	        /*	if (oliQualitatEjb.existenZonasActiusEnEstabliment(idEstablecimiento).booleanValue()){
	            ...
	            logger.info("Obtenint llistat de personal: " + llistat.size() + " registres trobats");

        	//}
	         
        } catch (Exception ex) {
            logger.error("Error obtenint llistat de personal", ex);
            ControllerUtils.saveMessageError(request, missatge("qualitat.descripcioPersonal.missatge.llistat.no", request));
        }
        ...
        for (int i=0; i<ids.length; i++) {
            ...
            if (id != null && !"".equals(id)) {
                try {
                	...
                	oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
                    oliQualitatEjb.personalEsborrar(Long.valueOf(id));

                } catch (Exception ex) {
                    logger.error("Error esborrant el personal [" + id + "]", ex);
                    exito = false;
                }
            }
        }
