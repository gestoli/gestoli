...
public class QualitatAPPCCPerillDelegate implements ApplicationContextAware {
	
	private static Logger logger = Logger.getLogger(QualitatAPPCCPerillDelegate.class);
   ...
   public ModelAndView list(
       ...
       try {
		...
		ResourceBundle bundle = ResourceBundle.getBundle("messages",new java.util.Locale(lang));
   		Collection llistat = oliQualitatEjb.aPPCCPerillCercaTotsPerEstabliment(idEstablecimiento);
   		
   		oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
       	myModel.put("idEstablecimiento",idEstablecimiento);
       	myModel.put("llistat", llistat);
       	myModel.put("path", "qualitat_APPCC.Perill");
           logger.info("Obtenint llistat de perills de l'APPCC: " + llistat.size() + " registres trobats");

       } catch (Exception ex) {
           logger.error("Error obtenint llistat de perills de l'APPCC", ex);
           ControllerUtils.saveMessageError(request, missatge("qualitat.appcc.perill.missatge.llistat.no", request));
       }
