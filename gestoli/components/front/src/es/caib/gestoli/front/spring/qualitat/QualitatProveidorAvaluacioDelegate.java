...
public class QualitatProveidorAvaluacioDelegate  implements ApplicationContextAware {
	private static Logger logger = Logger.getLogger(QualitatProveidorAvaluacioDelegate.class);
   ...
   public ModelAndView list(
       ...
       try {
		...
		ResourceBundle bundle = ResourceBundle.getBundle("messages",new java.util.Locale(lang));
   		...
   		Collection llistat = oliQualitatEjb.avaluacionsPerProveidor(idProveidor);
           ...
           logger.info("Obtenint llistat de ProveidorAvaluacio: " + llistat.size() + " registres trobats");

       } catch (Exception ex) {
           logger.error("Error obtenint llistat de ProveidorAvaluacio", ex);
           ControllerUtils.saveMessageError(request, missatge("qualitat.proveidor.avaluacio.missatge.llistat.no", request));
       }
        ...
        for (int i=0; i<ids.length; i++) {
            ...
            if (id != null && !"".equals(id)) {
                try {
                	...
                	logger.info("Eliminant el ProveidorAvaluacio [" + id + "]");
                	oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
                    oliQualitatEjb.proveidorAvaluacioEsborrar(Long.valueOf(id));

                } catch (Exception ex) {
                    logger.error("Error esborrant el ProveidorAvaluacio [" + id + "]", ex);
                    exito = false;
                }
            }
        }

        String forward = deleteView + "?idProveidor=" + request.getParameter("idProveidor");
