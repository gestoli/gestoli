...
//		Collection listaTrasllat = getEntradaTrasladosEntreDiasoTemporadas(temporadaId, dataInici, dataFi);
	...
	Collection col;
	SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
	String di = sdf.format(inici);
	String df = sdf.format(fi);	
		try {
			logger.debug("findHistòricTrasllatsPerEstabliment inici");
			String q = "select td.id, " +
					"'', " +
					"'', " +
					//"(select eo.nom from Establiment eo where eo.id = td.establimentByTdiCodeor.id), " +
					//"(select ed.nom from Establiment ed where ed.id = td.establimentByTdiCodede.id), " +
					"dip.codiAssignat, " +
					"td.dataAcceptarEnviament, td.quantitatEnviament, td.quantitatRetorn, " +
					"td.dataAcceptarRetorn, td.acceptatTrasllat, td.establimentByTdiCodeor.id," +
					"td.establimentByTdiCodede.id " +
					"from Establiment e, Trasllat td " +
					"left join td.diposits dip " +
					"left join td.traza tra " +
					"where (td.establimentByTdiCodeor.id = " + establimentId + " or td.establimentByTdiCodede.id = "+ establimentId +") " +
					"and dip.id = "+ dipositId +" " +
					"and (td.data >= '" + di + "' and td.data <= '" + df + "') " +
					"and td.traza.id = tra.id " +
					"and td.valid = true " +
					"group by td.id, dip.codiAssignat, td.establimentByTdiCodeor.id, " +
					"td.establimentByTdiCodede.id, " +
					"td.dataAcceptarEnviament, td.quantitatEnviament, td.dataAcceptarRetorn, td.quantitatRetorn,td.acceptatTrasllat " +
					"order by td.id desc";	
			
			col = getHibernateTemplate().find(q);
			for(Iterator i=col.iterator();i.hasNext();){
				Object[] result = (Object[]) i.next();
				String origen = "select e.nom from Establiment e where e.id = "+result[9];
				String desti = "select e.nom from Establiment e where e.id= "+result[10];
				List or = getHibernateTemplate().find(origen);
				result[1]= or.get(0);
				List de = getHibernateTemplate().find(desti);
				result[2] = de.get(0);
			}
		} catch (HibernateException ex) {
