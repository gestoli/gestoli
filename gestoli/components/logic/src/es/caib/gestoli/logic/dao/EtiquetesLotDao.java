	...
	public Collection findEtiquetesLotEmpradesByLot(Long idLot, Boolean esEcologic) {
		try {
			String q =	"from EtiquetesLot el " +
						...
						if(esEcologic) q += " and el.esEcologic = true ";
						if(!esEcologic) q +=  " and el.esEcologic != true || el.esEcologic = null ";
						q += "order by el.id asc ";
			return getHibernateTemplate().find(q);
		} catch (HibernateException ex) {
			throw new InfrastructureException(ex);
		}
	...
	public Collection findEtiquetesLotDisponiblesByEstabliment(Long idEstabliment) {
		try {
			String q =	"from EtiquetesLot el " +
						...
						q += "where el2.rangOriginal.id = el.id) ";
						q += "order by el.id asc ";
		...
		for(Object o: etiquetesEstabliment){
			...
			if(esEcologic){
				if (e.getOlivaTaula()!=null && e.getOlivaTaula()!=false && olivaTaula){
					if(e.getEsEcologic()!= null && e.getEsEcologic()){
						resultat.add(e);
					}
