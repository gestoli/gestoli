 ...
 * 
 */
public class HistoricPlantacioDao {
	private static Logger logger = Logger.getLogger(HistoricPlantacioDao.class);
	...
	public Collection findAll() throws InfrastructureException {
		...
		try {
			...
			col = getHibernateTemplate().find("from HistoricPlantacio as pl order by pl.id"); 
	...
	public boolean existenPlantacionesAsociadasFincas(Long idFinca) throws InfrastructureException {
		...
		try {
			...
			String q = "select count(pla.id) from HistoricPlantacio pla where pla.finca.id = " + idFinca + " ";
	...
	public List getByFincaId(Long fincaId) throws InfrastructureException {
		...
		try {
			...
			String q =	"from HistoricPlantacio as pl " +
	...
	public List getActivasByFincaId(Long fincaId) throws InfrastructureException {
		...
		try {
			...
			String q = "from HistoricPlantacio as pl where pl.actiu = true and pl.finca.id = " + fincaId;
			plantaciones = getHibernateTemplate().find(q);
	...
	public List getActivasConDescomposicionByFincaId(Long fincaId) throws InfrastructureException {
		...
		try {
			...
			String q = "select distinct dpl.plantacio from HistoricDescomposicioPlantacio as dpl" +
	...
	public List getActivasByOlivicultorId(Long olivicultorId) throws InfrastructureException {
		...
		try {
			...
			String q = "from HistoricPlantacio as pl where pl.actiu = true" +
			" and pl.finca in( from HistoricFinca as finca where finca.olivicultor.id = "+olivicultorId+" and finca.actiu = true)" ;
	...
	public Collection findAllIdPlantacioActivas(Long idFinca) throws InfrastructureException {
		...
		try {
			...
			String q = "select plantacio.id from HistoricPlantacio as plantacio where plantacio.actiu = true and plantacio.finca.id = "+idFinca+" order by plantacio.id"; 
	 ...
	 * @throws InfrastructureException
	 */
	public void makePersistent(HistoricPlantacio plantacio) throws InfrastructureException {
		try {
	 ...
	 * @throws InfrastructureException
	 */
	public void makePersistent(HistoricPlantacio plantacio, Session session) throws InfrastructureException {
		try {
			logger.debug("makePersistent ini");
	 ...
	 * @throws InfrastructureException
	 */
	public void makeTransient(HistoricPlantacio plantacio) throws InfrastructureException {
		try {
	...
	public Plantacio getById(Long id) throws InfrastructureException {
		...
		try {
			...
			plantacio = (Plantacio)getHibernateTemplate().load(HistoricPlantacio.class, id);	
		} catch (HibernateException ex) {
	...
	public List findPlantacionsDeBaixa() throws InfrastructureException {
		try {
			String q = "from HistoricPlantacio p where p.deBaixa = true";
