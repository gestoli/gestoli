...
public class AutocontrolOlivaDao {
	 ...
	 * @throws InfrastructureException
	 */
	public List findByBota(Long botaId) throws InfrastructureException {
		List autocontrols;
		try {
			logger.debug("findByBota ini");
			
			String q = "from 	AutocontrolOliva aut " +
					   "where 	aut.bota = " + botaId +
					   " and 	aut.apta = true " +
					   "order by aut.data desc";
			
			autocontrols = getHibernateTemplate().find(q);
		} catch (HibernateException ex) {
			logger.error("findByBota failed", ex);
			throw new InfrastructureException(ex);
		}
		logger.debug("findByBota fin");
		return autocontrols;
	}
...
//	}
	...
	/**
	 * Devuelve la ultima analitica que se hizo a una bota
	 * @param 
	 * @return
	 * @throws InfrastructureException
	 */
	public AutocontrolOliva findUltimaByLot(Long lotId) throws InfrastructureException {
		...
		try {
			...
			"aut.bota = "+ lotId +" and aut.valid = true " +
