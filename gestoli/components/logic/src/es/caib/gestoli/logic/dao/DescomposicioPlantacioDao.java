...
//			session.saveOrUpdate(descomposicioplantacio);
	 ...
	 * @throws InfrastructureException
	 */
	public Collection getSuperficiesVarietatsByCampanya(Long[] olivicultorsId, Long campanyaId, Boolean taula, Boolean experimental) throws InfrastructureException {
		...
		try {
			...
			if (cam.getDataFi() == null) {
				q =	"select dpo.varietatOliva.id as varietat, sum(dpo.superficie) as superficie, sum(dpo.numeroOliveres) as oliveres " +
					...
					"and    dpo.plantacio.finca.olivicultor.altaDO = true " +
					(!taula && experimental?"and (dpo.varietatOliva.experimental = true and dpo.varietatOliva.olivaTaula=false )" :"") +
					(taula && !experimental?"and (dpo.varietatOliva.experimental = false and dpo.varietatOliva.olivaTaula = true )" :"") +
					(!taula && !experimental?"and (dpo.varietatOliva.experimental = false and dpo.varietatOliva.olivaTaula = false )" :"") +
