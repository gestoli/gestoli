...
<html>
<head>
<title><fmt:message key="consulta.oliEntradaTraslado.titol" /></title>
...
<body>
<form id="formulario" action="ConsultaOliEntradaTrasladoLlistat.html"
...
<c:if test="${llistat != null && not empty llistat}">
	...
	<display:table name="llistat" requestURI="" id="trasllat" export="true"
		...
		<display:column
			titleKey="consulta.oliEntradaTraslado.camp.retornatEstablimentOrigen"
			sortable="true" headerClass="ancho75">
			<c:choose>
				<c:when
					test="${not empty trasllat.retornatEstablimentOrigen && trasllat.retornatEstablimentOrigen}">
		...
		<display:column headerClass="ancho20 paddingCelda final">
			<a
				href="ConsultaTrazabilitat.html?tipus=<c:out value="${trazaTipusEntradaVolant}"/>&amp;id=<c:out value="${trasllat.id}"/>"><img
		...
		<display:setProperty name="export.excel.filename" value="EntradesTrasllat.xls" />
