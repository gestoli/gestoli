 ...
 */
public class SortidaBotaGranelDao {
	private static Logger logger = Logger.getLogger(SortidaBotaGranelDao.class);
	...
	public Collection findAll(Boolean valid) throws InfrastructureException {
		...
		try {
			...
			String q = "from SortidaBotaGranel as slot where 1 = 1 ";
	...
	public Collection findByEstablecimiento(Long estId, Boolean valid) throws InfrastructureException {
		...
		try {
			...
			String q = "from SortidaBotaGranel as slot where " +
	 ...
	 * @throws InfrastructureException
	 */
	public SortidaBotaGranel getById(Long id) throws InfrastructureException {
		SortidaBotaGranel sortida;
		try {
			logger.debug("getById ini");
			sortida = (SortidaBotaGranel)getHibernateTemplate().load(SortidaBotaGranel.class, id);	
		} catch (HibernateException ex) {
	 ...
	 * @throws InfrastructureException
	 */
	public SortidaBotaGranel findByTraza(Long idTraza, Boolean valid) throws InfrastructureException {
		List col;
		SortidaBotaGranel sortidaBotaGranel = null;
		try {
			logger.debug("findByTraza ini");
			String q = "from SortidaBotaGranel slo where slo.traza.id="+idTraza;
			if (valid != null && valid.booleanValue()) q += " and slo.valid = true ";
			if (valid != null && !valid.booleanValue()) q += " and slo.valid = false ";
			col = getHibernateTemplate().find(q);
			if(col!= null && col.size()>0){
				sortidaBotaGranel = (SortidaBotaGranel)col.get(0);
			}
		} catch (HibernateException ex) {
	 ...
	 * @throws InfrastructureException
	 */
	public void makePersistent(SortidaBotaGranel sortida) throws InfrastructureException {
		try {
	 ...
	 * @throws InfrastructureException
	 */
	public void makeTransient(SortidaBotaGranel sortida)
	throws InfrastructureException {
		try {
	...
	/**
	 * Devuelve SortidaVendaOlivaTaulaBotaGranelEntreDiasEnEstablecimiento por establecimiento y entre 2 fechas
	 * @param 
	 * @return
	 * @throws InfrastructureException
	 */
	public Collection getSortidaVendaOlivaTaulaBotaGranelEntreDiasEnEstablecimiento(Date finicio, Date ffin, Long estId, Boolean valid) throws InfrastructureException {
		Collection col;
		SimpleDateFormat df = new SimpleDateFormat("dd/MM/yyyy");
		String fi = df.format(finicio);
		String ff = df.format(ffin);
		try {
			logger.debug("getSortidaVendaOlivaTaulaBotaGranelEntreDiasEnEstablecimiento ini");
			String q = 	" from SortidaBotaGranel slo " +
						"where slo.vendaData >= '"+fi+"' " +
						" and slo.vendaData <= '"+ff+"' " +
						" and slo.accion = 'v' ";
			if (estId != null) q += "and slo.bota.establiment.id = " +estId+ " ";
			if (valid != null && valid.booleanValue()) q += " and slo.valid = true ";
			if (valid != null && !valid.booleanValue()) q += " and slo.valid = false ";
			q += " order by slo.vendaData desc";
			col = getHibernateTemplate().find(q);
		} catch (HibernateException ex) {
			logger.error("getSortidaVendaOlivaTaulaBotaGranelEntreDiasEnEstablecimiento failed", ex);
			throw new InfrastructureException(ex);
		}
		logger.debug("getSortidaVendaOlivaTaulaBotaGranelEntreDiasEnEstablecimiento fin");
		return col;
	}
	 ...
	 * @throws InfrastructureException
	 */
	public Long getSumaSortidesOlivaTaulaBotaGranelFinsData(Long idPartida, Date data, Boolean valid) {
		Long kilos = 0L;
		Collection col;
		SimpleDateFormat df = new SimpleDateFormat("dd/MM/yyyy");
		String fi = df.format(data);
		try {
			logger.debug("getSumaSortidesOlivaTaulaBotaGranelFinsData ini");
			String q = 	" select sum(slo.vendaNumeroBotelles) " +
						"from SortidaBotaGranel slo " +
			...
			logger.error("getSumaSortidesOlivaTaulaBotaGranelFinsData failed", ex);
			throw new InfrastructureException(ex);
		}
		logger.debug("getSumaSortidesOlivaTaulaBotaGranelFinsData fin");
	 ...
	 * @throws InfrastructureException
	 */
	public Double getSumaSortidesOlivaTaulaBotaGranelFinsData(Long[] idPartides, Date data, Boolean valid) {
		Double kilos = 0.0;
		Collection col;
		SimpleDateFormat df = new SimpleDateFormat("dd/MM/yyyy");
		String fi = df.format(data);
