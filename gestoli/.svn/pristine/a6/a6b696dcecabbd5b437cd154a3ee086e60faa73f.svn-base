...
public class GenerarPdfAcompanyamentFonoll extends AbstractPdfView {
	private static Logger logger = Logger.getLogger(GenerarPdfAcompanyamentFonoll.class);
	...
	protected void buildPdfDocument(
		...
		PartidaFonoll pf = oliInfraestructuraEjb.partidaFonollAmbId(Long.parseLong(idPartida));
		
		String titular = pf.getTitular();
		String data = dateF.format(pf.getData());
			...
			PdfReader reader = new PdfReader(this.getClass().getClassLoader().getResourceAsStream("es/caib/gestoli/logic/resources/documentAcompanyamentFonoll.pdf"));
			
			PdfStamper stamp = new PdfStamper(reader, baos);
			AcroFields formulariPdf = stamp.getAcroFields();
			
			formulariPdf.setField("DOCNUM", pf.getCodi());
			formulariPdf.setField("TITULAR", titular);
			formulariPdf.setField("DATA", data);
			formulariPdf.setField("POLIGON", pf.getPoligon());
			formulariPdf.setField("PARCELA", pf.getParcela());
			formulariPdf.setField("MUNICIPI", pf.getMunicipi().getNom());
			formulariPdf.setField("KG", numberDecimalFormat.format(pf.getKgInici()));
			...
			response.setHeader("Content-Disposition","attachment; filename=documentAcompanyamentFonoll.pdf");
