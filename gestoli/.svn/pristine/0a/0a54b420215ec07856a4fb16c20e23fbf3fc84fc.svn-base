...
public class QualitatAiguaControlAnaliticFormController extends SimpleFormController {

	private static Logger logger = Logger.getLogger(QualitatAiguaControlAnaliticFormController.class);
	...
	throws ServletException {
		QualitatAiguaControlAnaliticCommand qualitatAiguaControlAnaliticCommand = (QualitatAiguaControlAnaliticCommand)command;
		...
		QualitatAiguaControlAnalitic desc = est.getAiguaControlAnalitic();
		try {
			...
			if (action != null && action.equals("delete")){
				...
				oliQualitatEjb.aiguaControlAnaliticVerificacioEsborrar(id);
				ControllerUtils.saveMessageInfo(request, missatge("qualitat.plaAigua.missatge.esborrar.ok"));
			} else {
				
				QualitatAiguaControlAnalitic control= new QualitatAiguaControlAnalitic();
				
					if (qualitatAiguaControlAnaliticCommand.getResponsableId()!=null){
						control.setResponsable(oliQualitatEjb.personalAmbId(qualitatAiguaControlAnaliticCommand.getResponsableId()));}
					control.setFrequencia(qualitatAiguaControlAnaliticCommand.getFrequencia());
				...
				if (desc != null) {
					...
					oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
					oliQualitatEjb.actualitzarAiguaControlAnalitic(control);
					ControllerUtils.saveMessageInfo(request, missatge("qualitat.plaAigua.missatge.crear.ok"));
				} else {
					oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
					oliQualitatEjb.crearAiguaControlAnalitic(control);
			...
			if (action.equals("delete")){
				...
				ControllerUtils.saveMessageError(request, missatge("qualitat.plaAigua.missatge.crear.no"));
			} else {
				logger.error("Error modificant la descripció de la instal.lació", ex);
	...
	protected Map referenceData(
      ...
      //frecuencia 
		...
		Collection<QualitatAiguaControlAnaliticVerificacio> resultats = new ArrayList();
		if (est.getAiguaControlAnalitic()!= null) {
			oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
			resultats = oliQualitatEjb.aiguaControlAnaliticVerificacioPerControl(est.getAiguaControlAnalitic().getId());
		}
		for (Object net: resultats){
    		if (((Boolean)true).equals(((QualitatAiguaControlAnaliticVerificacio)net).getSatisfactori()))
    			((QualitatAiguaControlAnaliticVerificacio)net).setSatisfactoriString(bundle.getString("qualitat.plaNeteja.verificacio.satisfactoria.si"));
    		else ((QualitatAiguaControlAnaliticVerificacio)net).setSatisfactoriString(bundle.getString("qualitat.plaNeteja.verificacio.satisfactoria.no"));
    	}
	...
	protected Object formBackingObject(HttpServletRequest request) throws ServletException {
		try {
			QualitatAiguaControlAnaliticCommand command = new QualitatAiguaControlAnaliticCommand();
			if (!isFormSubmission(request)) {
				try {
					Establiment establiment = (Establiment)request.getSession().getAttribute(establimentSessionKey);
					
					if (establiment.getAiguaControlAnalitic() == null){
				   		oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
				   		QualitatAiguaControlAnalitic aiguaControlAnalitic = oliQualitatEjb.aiguaControlAnaliticByEstabliment(establiment.getId());
						establiment.setAiguaControlAnalitic(aiguaControlAnalitic);
					}
					if (establiment.getAiguaControlAnalitic()!=null) {
						Long id = establiment.getAiguaControlAnalitic().getId();
						oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
						QualitatAiguaControlAnalitic aiguaControlAnalitic = oliQualitatEjb.aiguaControlAnaliticAmbId(id);
						command.fromQualitatAiguaControlAnalitic(aiguaControlAnalitic);
					}
