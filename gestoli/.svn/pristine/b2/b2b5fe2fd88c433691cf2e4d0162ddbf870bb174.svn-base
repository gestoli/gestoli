...
public class QualitatPlaControlTransportDelegate  implements ApplicationContextAware {
	
	private static Logger logger = Logger.getLogger(QualitatPlaControlTransportDelegate.class);
    ...
    public ModelAndView list(
        ...
        try {
    		...
    		Establiment est = (Establiment)session.getAttribute(establimentSessionKey);
	   		...
	   		Collection llistat = oliQualitatEjb.plansControlTransportPerEstabliment(idEstablecimiento);
	   		
	   		for (Object net: llistat){
        		if (((Boolean)true).equals(((QualitatPlaControlTransport)net).getEstibaCorrecta()))
        			((QualitatPlaControlTransport)net).setEstibaCorrectaString(bundle.getString("qualitat.plaNeteja.verificacio.satisfactoria.si"));
        		else ((QualitatPlaControlTransport)net).setEstibaCorrectaString(bundle.getString("qualitat.plaNeteja.verificacio.satisfactoria.no"));
        		if (((Boolean)true).equals(((QualitatPlaControlTransport)net).getNetejaCorrecta()))
        			((QualitatPlaControlTransport)net).setNetejaCorrectaString(bundle.getString("qualitat.plaNeteja.verificacio.satisfactoria.si"));
        		else ((QualitatPlaControlTransport)net).setNetejaCorrectaString(bundle.getString("qualitat.plaNeteja.verificacio.satisfactoria.no"));
        	}
            ...
            logger.info("Obtenint llistat de PlaControlTransport: " + llistat.size() + " registres trobats");

        } catch (Exception ex) {
            logger.error("Error obtenint llistat de PlaControlTransport", ex);
            ControllerUtils.saveMessageError(request, missatge("qualitat.plaControlTransport.missatge.llistat.no", request));
        }
        ...
        for (int i=0; i<ids.length; i++) {
            ...
            if (id != null && !"".equals(id)) {
                try {
                	...
                	logger.info("Eliminant el PlaControlTransport [" + id + "]");
                	oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
                    oliQualitatEjb.plaControlTransportEsborrar(Long.valueOf(id));

                } catch (Exception ex) {
                    logger.error("Error esborrant el PlaControlTransport [" + id + "]", ex);
                    exito = false;
                }
            }
        }
