 ...
 * 
 */
public class GestorDocumentalCategoriaInformacioDao {
	private static Logger logger = Logger.getLogger(GestorDocumentalCategoriaInformacioDao.class);
	...
	public Collection findAll() throws InfrastructureException {
		...
		try {
			...
			col = getHibernateTemplate().find("from GestorDocumentalCategoriaInformacio as catInf order by catInf.nom"); 
	 ...
	 * @throws InfrastructureException
	 */
	public GestorDocumentalCategoriaInformacio getById(Integer id) throws InfrastructureException {

		List categorias;
		try {
			logger.debug("getById ini");
			String q = "from GestorDocumentalCategoriaInformacio as cat where cat.id = " + id;
			categorias = getHibernateTemplate().find(q);
			if (categorias.size() > 0) {
				logger.debug("getById fin");
				return (GestorDocumentalCategoriaInformacio)categorias.get(0);
			}
		} catch (HibernateException ex) {
	...
	public boolean existenInformacionesAsociadasToCategoria(Integer id) throws InfrastructureException {
		...
		try {
			...
			String q = "from GestorDocumentalInformacio as inf where inf.gestorDocumentalCategoriaInformacio.id = " + id;
	...
	public boolean existeOtraCategoriaConNombre(String nom, Integer id) throws InfrastructureException {
		...
		try {
			...
			String q = "from GestorDocumentalCategoriaInformacio as cat where cat.nom = :nom and cat.id != " + id;
	 ...
	 * @throws InfrastructureException
	 */
	public void makePersistent(GestorDocumentalCategoriaInformacio catInf) throws InfrastructureException {
		try {
	 ...
	 * @throws InfrastructureException
	 */
	public void makePersistent(GestorDocumentalCategoriaInformacio catInf, Session session) throws InfrastructureException {
		try {
	 ...
	 * @throws InfrastructureException
	 */
	public void makeTransient(GestorDocumentalCategoriaInformacio catInf)
	throws InfrastructureException {
		try {
