 ...
 *
 */
public class ConsultaOlivaTaulaComercialitzadaEstablimentLlistatController extends SimpleFormController {

    /*
     * Objeto para escribir mensajes de log.
     */
	private static Logger logger = Logger.getLogger(ConsultaOlivaTaulaComercialitzadaEstablimentLlistatController.class);
    ...
    private String trazaTipusOlivaTaulaComercialitzada;
    ...
    throws ServletException {
    	ConsultaOlivaTaulaComercialitzadaEstablimentLlistatCommand consulta = (ConsultaOlivaTaulaComercialitzadaEstablimentLlistatCommand) command;
    	...
    	try {
    		...
    		myModel.put("path", "consultar_olivaTaulaComercialitzada");
    		if (consulta.getFromEstabliment() != null && consulta.getFromEstabliment().equalsIgnoreCase("true")){	
	    		...
	    		if(request.getParameter("fromEstabliment")!= null){
	    			...
	    			if(establiment.getNom()!= null && !establiment.getNom().equalsIgnoreCase("")){
	    				myModel.put("path", "consulta_olivaTaulaComercialitzadaEstabliment");
    ...
    protected Map referenceData(
        ...
        try {
        	...
        	for(Object o: llistatEstabliments){
        		...
        		Boolean olivaTaula = e.getOlivaTaula()==null ? false : e.getOlivaTaula();
        		if (olivaTaula) {
	        		Integer potsVerda = 0;
	    		   	Integer potsTrencada= 0;
	    		   	Integer potsNegra = 0;
			        ...
			        Collection listaLotes = oliConsultaEjb.getSortidaLotVendaOlivaTaulaEntreDiasEnEstablecimiento(dataInici, dataFi, e.getId());
			        
			        for(Object obj: listaLotes){
			        	SortidaLot sortidalot = (SortidaLot)obj;
			        	Boolean lotOlivaTaula = sortidalot.getLot().getOlivaTaula()==null ? false : sortidalot.getLot().getOlivaTaula();
			        	if (lotOlivaTaula) {
				        	...
				        	if (botelles_sortides > 0) {
				        		if(sortidalot.getLot().getTipusOlivaTaula()==Constants.TIPUS_OLIVA_TAULA_VERDA){
			    		   			potsVerda += sortidalot.getVendaNumeroBotelles(); 
			    		   		} else if(sortidalot.getLot().getTipusOlivaTaula()==Constants.TIPUS_OLIVA_TAULA_TRENCADA){ 
			    		   			potsTrencada += sortidalot.getVendaNumeroBotelles(); 
			    		   		} else if(sortidalot.getLot().getTipusOlivaTaula()==Constants.TIPUS_OLIVA_TAULA_NEGRA){ 
			    		   			potsNegra += sortidalot.getVendaNumeroBotelles(); 
			    		   		}
				        	}
			        	}
	    		   	}
    		...
    		if(!errors.hasErrors()){
    			...
    			myModel.put("trazaTipusOlivaTaulaComercialitzada", trazaTipusOlivaTaulaComercialitzada);
    		}
    ...
    protected Object formBackingObject(HttpServletRequest request) throws ServletException {
    	ConsultaOlivaTaulaComercialitzadaEstablimentLlistatCommand consulta = (ConsultaOlivaTaulaComercialitzadaEstablimentLlistatCommand)request.getSession().getAttribute("consulta");
    	if (consulta == null) 
    		consulta = new ConsultaOlivaTaulaComercialitzadaEstablimentLlistatCommand();
    	else 
	...
	/**
	 * Metodo 'setTrazaTipusOlivaTaulaComercialitzada'
	 * @param trazaTipusOlivaTaulaComercialitzada
	 */
	public void setTrazaTipusOlivaTaulaComercialitzada(String trazaTipusOlivaTaulaComercialitzada) {
		this.trazaTipusOlivaTaulaComercialitzada = trazaTipusOlivaTaulaComercialitzada;
	}
