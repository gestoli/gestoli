...
<html>
	<head>
		<title><fmt:message key="proces.sortidaOlivaCruaGranel.titol" /></title>
...
<%		} %>
	...
	<body>
		<form id="formulario" name="procesSortidaOlivaTaulaCruaGranelForm" action="ProcesSortidaOlivaTaulaCruaGranelForm.html?accio=${venda}" method="post" class="extended seguit">
			...
			<c:if test="${tipusSortida == 'p'}">
	
				<c:import url="comu/CampFormulari.jsp">
					<c:param name="tipus" value="hidden" />
					<c:param name="path" value="formData.idPartidaOliva" />
					<c:param name="camp" value="idPartidaOliva" />
					<c:param name="name" value="idPartidaOliva" />
					<c:param name="value" value="${partidaOliva.id}" />
				</c:import>
				...
				<c:import url="comu/CampFormulari.jsp">
					...
					<c:param name="name" value="idEstabliment" />
					<c:param name="value" value="${establiment.id}" />
				</c:import>
								
<%-- 				<c:forEach var="partida" items="${partidaOliva}" varStatus="status"> --%>
					...
					<div class="cajaTabla">
						<div class="layoutSombraTabla">
							...
							<table cellpadding="0" cellspacing="0" class="listadoEstrecho noRoll">
								<thead>
									<tr>
										<th colspan="2" class="unicoHead">Id Partida: <c:out value=" ${partidaOliva.id}" /></th>
									</tr>
								</thead>
								<tbody>
									<tr>
										<td><fmt:message key="proces.envasatOliva.camp.kgRestants" /></td>
										<td class="boldtd"><c:out value="${partidaOliva.quantitat}" /></td>
									</tr>
								</tbody>
							</table>
						</div>
					...
					<c:import url="comu/CampFormulari.jsp">
						...
						<c:param name="title"><fmt:message key="sortidaLot.camp.kilos" /></c:param>
...
<%-- 				</c:forEach> --%>
		...
		<form id="tornarForm" action="ProcesInici.html" class="seguit">
			<input type="hidden" id="tipusProces" name="tipusProces" value="24" />
