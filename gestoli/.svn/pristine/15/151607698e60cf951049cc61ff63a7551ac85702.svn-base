 ...
 */
public class EtiquetatgeDelegate implements ApplicationContextAware {
	private static Logger logger = Logger.getLogger(EtiquetatgeDelegate.class);
    ...
    public ModelAndView list(
        ...
        try {
        	...
        	Collection llistat = oliInfraestructuraEjb.etiquetatgeCercaTots();
        	myModel.put("llistat", llistat);        	
            logger.info("Obtenint llistat de etiquetatges: " + llistat.size() + " registres trobats");
        } catch (Exception ex) {
            logger.error("Error obtenint llistat de etiquetatge", ex);
            ControllerUtils.saveMessageError(request, missatge("etiquetatge.missatge.llistat.no", request));
        }
