...
<form id="formulario" action="<%=request.getContextPath()%>/AltresConsultesTrazabilidadResumida.html" method="post" class="extended seguit">
...
<c:if test="${llistat != null && not empty llistat}">
	<div id="listado"><%-- Tabla de resultados --%>
		...
		<display:table name="llistat" requestURI="" id="partida" export="true" sort="list" cellpadding="0" cellspacing="0" class="listadoAncho selectable">
			<display:column titleKey="consulta.traza.codiAssignat" sortable="true" sortProperty="nom">
				<a href="AltresConsultesTrazabilidadPartidaOli.html?id=<c:out value="${partida.nom}"/><c:if test="${not empty param.idProductor }">&idProductor=<c:out value="${param.idProductor}"/></c:if>"></a>
			</display:column>
			<display:column titleKey="consulta.traza.establiment" property="establiment.nom" sortable="true" sortProperty="establiment.nom"/>
			<%--display:column titleKey="consulta.traza.litros" sortable="true" sortProperty="litres" headerClass="ancho50">
