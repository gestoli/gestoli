 ...
 * @author Raul Morales
 */
public class ConsultaEntradaOlivaTaulaController extends SimpleFormController {

    /*
     * Objeto para escribir mensajes de log.
     */
	private static Logger logger = Logger.getLogger(ConsultaEntradaOlivaTaulaController.class);
    ...
    throws ServletException {
    	ConsultaEntradaOlivaTaulaCommand consulta = (ConsultaEntradaOlivaTaulaCommand) command;
    ...
    protected Map referenceData(
        ...
        try {
        	...
        	if (Util.isDataCorrecta(request.getParameter("dataInici"), "dd/MM/yyyy")
        		...
        		boolean update = supdate != null;
        		
        		Collection llistat = oliConsultaEjb.entradesOlivaConsulta(dataInici, dataFi, idEstabliment);;
        		for(Object o: lista){
        			PartidaOliva po = (PartidaOliva)o;
        			
        			if (!po.getEsOlivaDeTaula()){
        				llistat.remove(o);
        			} else if(update) {
        				if (po.getQuantitat() == null) {
        					if (po.getElaboracioOlivaTaula() != null)
        						po.setQuantitat(0.0);
        					else {
        						Double quantitatUsada = 0.0;
        						Set<PartidaElaboracio> pelas = po.getPartidaElaboracions();
        						for (PartidaElaboracio pela: pelas){
        							quantitatUsada += pela.getQuantitat();
        						}
        						po.setQuantitat(po.getTotalKilos() - quantitatUsada);
        					}
        					oliInfraestructuraEjb.setHibernateTemplate(getHibernateTemplate());
        					oliInfraestructuraEjb.partidaOlivaModificar(po);
        				}
        			}
        		}
...
//        	ControllerUtils.saveMessageError(request, "No s'ha obtingut cap registre");
    ...
    protected Object formBackingObject(HttpServletRequest request) throws ServletException {
    	ConsultaEntradaOlivaTaulaCommand consulta = (ConsultaEntradaOlivaTaulaCommand)request.getSession().getAttribute("consulta");
    	if (consulta == null) 
    		consulta = new ConsultaEntradaOlivaTaulaCommand();
    	else 
