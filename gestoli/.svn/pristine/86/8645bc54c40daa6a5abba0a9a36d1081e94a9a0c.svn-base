/**
 * ProcesMoureOliFormController.java
 */
 ...
 */
public class ProcesMoureOliFormController extends SimpleFormController {

    private static Logger logger = Logger.getLogger(ProcesMoureOliFormController.class);
    ...
    throws ServletException {
    	...
    	ProcesMoureOliCommand pMoureOli = (ProcesMoureOliCommand)command;    	
    	
    	Map myModel = new HashMap();
    	Establiment establiment = (Establiment) request.getSession().getAttribute(establimentSessionKey);
    	Long idMoureOli = null;
    	
		if (isCreate(request)) {
			try{ 
				
				//Posem el volum a traslladar dins el dipòsit
				for(int i=0; i<pMoureOli.getDipositsOrigen().length; i++){
					Double volTrasllat = pMoureOli.getLitros()[i];
					pMoureOli.getDipositsOrigen()[i].setVolumTrasllat(volTrasllat);
				}
				...
				oliProcessosEjb.generarMoureOli(pMoureOli.getData(), pMoureOli.getDipositsOrigen(), pMoureOli.getEstablimentDesti(), establiment, rolDoControlador, rolDoGestor, emailFrom);
						
				ControllerUtils.saveMessageInfo(request, missatge("proces.moureOli.missatge.ok"));
				
	    	} catch (Exception e) {
	    		 logger.error("Error processant el MoureOli de l'oli", e);
	             ControllerUtils.saveMessageError(request, missatge("proces.moureOli.missatge.no"));
			}
		}
	...
	protected Map referenceData(
    	...
    	//Emplenam els litres i els kilos
    	if (!isFormSubmission(request)) {
	    	if(origen != null){
		    	Double[] litres = new Double[origen.size()];
		    	Double[] kilos = new Double[origen.size()];
		    	
		    	int i = 0;
		    	for(Object o: origen){
		    		Diposit d = (Diposit)o;
		    		litres[i] = d.getVolumActual();
		    		kilos[i] = d.getDisponibleOliQuilos();
		    		i++;
		    	}
		    	
		    	ProcesMoureOliCommand c = (ProcesMoureOliCommand) command;
		    	c.setLitros(litres);
		    	c.setKilos(kilos);
		    	
		    	myModel.put("formData", c);
	    	}
    	}
    ...
    protected Object formBackingObject(HttpServletRequest request) throws ServletException {
    	...
    	ProcesMoureOliCommand pMoureOli = new ProcesMoureOliCommand();
    	...
    	try {    		    		
			...
			//			if (!isFormSubmission(request)) {
			//					pMoureOli.setData(new Date());			
			//			} 
			
			Diposit[] dipositsOrigen = new Diposit[origenList.size()];
			Double[] litres = new Double[origenList.size()];
	    	Double[] kilos = new Double[origenList.size()];
			
			for (int i = 0; i < dipositsOrigen.length; i++) {
				dipositsOrigen[i] = new Diposit();
				litres[i] = new Double(0);
				kilos[i] = new Double(0);
			}
			
			pMoureOli.setDipositsOrigen(dipositsOrigen);
			pMoureOli.setLitros(litres);
			pMoureOli.setKilos(kilos);
