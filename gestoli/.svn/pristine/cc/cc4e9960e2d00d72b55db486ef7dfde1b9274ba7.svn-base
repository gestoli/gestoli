...
//			}
	...
	/**
	 * Recupera todos los olivicultores dados de alta en la DO de la campanya dada productors RT
	 * @param campanyaId
	 * @return
	 * @throws InfrastructureException
	 */
	...
	public Collection findAllIdAltaDOByCampanyaRA(Long campanyaId) throws InfrastructureException {
		...
		try {
			...
			Campanya cam = (Campanya)campanyes.get(0);
			if (cam.getDataFi() == null) {
				col = getHibernateTemplate().find("select oli.id from Olivicultor oli where oli.altaDO = true and oli.campanya.id = "+campanyaId+" and (oli.codiDO is not null or (oli.codiDOExperimental is not null and oli.codiDO is not null)) and oli.codiDOPOliva is null order by oli.codiDO");
			} else {
				col = getHibernateTemplate().find("select holi.id from HistoricOlivicultor holi where holi.altaDO = true and holi.campanya.id = "+campanyaId+" and (holi.codiDO is not null or (holi.codiDOExperimental is not null and holi.codiDO is not null)) and holi.codiDOPOliva is null order by holi.codiDO");
			}
		} catch (HibernateException ex) {
	...
	public Collection findAllIdAltaDOByCampanyaRE(Long campanyaId) throws InfrastructureException {
		...
		try {
			...
			Campanya cam = (Campanya)campanyes.get(0);
			if (cam.getDataFi() == null) {
				col = getHibernateTemplate().find("select oli.id from Olivicultor oli where oli.altaDO = true and oli.campanya.id = "+campanyaId+" and (oli.codiDOExperimental is not null or (oli.codiDOExperimental is not null and oli.codiDO is not null)) and oli.codiDOPOliva is null order by oli.codiDOExperimental");
			} else {
				col = getHibernateTemplate().find("select holi.id from HistoricOlivicultor holi where holi.altaDO = true and holi.campanya.id = "+campanyaId+" and (holi.codiDOExperimental is not null or (holi.codiDOExperimental is not null and holi.codiDO is not null)) and holi.codiDOPOliva is null order by holi.codiDOExperimental");
			}
		} catch (HibernateException ex) {
	...
	/**
	 * Recupera tots els ids dels olivicultors dados de alta en la DO de la campanya dada  productors RT
	 * @param campanyaId
	 * @return
	 * @throws InfrastructureException
	 */
