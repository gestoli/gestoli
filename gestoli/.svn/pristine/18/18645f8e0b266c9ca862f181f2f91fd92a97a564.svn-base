 ...
 * 
 */
public class LotDelegate implements ApplicationContextAware {
	
	private static Logger logger = Logger.getLogger(LotDelegate.class);
    ...
    public ModelAndView list(
        ...
        try {
        	//if (request.isUserInRole(rolDoGestor)) {
	        	...
	        	Collection llistat = oliInfraestructuraEjb.lotCercaTotsPerEstabliment(idEstablecimiento);
	        	oliInfraestructuraEjb.setHibernateTemplate(getHibernateTemplate());
	        	...
	        	myModel.put("path", "lista_lots");
	            logger.info("Obtenint llistat de lots: " + llistat.size() + " registres trobats");

        	//}
	         
        } catch (Exception ex) {
            logger.error("Error obtenint llistat de lots", ex);
            ControllerUtils.saveMessageError(request, missatge("lot.missatge.llistat.no", request));
        }
...
//        boolean exito = true;
        if (id != null && !"".equals(id)) {
            try {
            	...
            	oliInfraestructuraEjb.setHibernateTemplate(getHibernateTemplate());
                if (oliInfraestructuraEjb.existeixenEntradesLotAssociadesLot(lid)) {
                	ControllerUtils.saveMessageError(request, missatge("diposit.missatge.esborrar.no.entradaDiposit", request));
                } else if (oliInfraestructuraEjb.existeixenSortidesLotAssociadesLot(lid)) {
                	ControllerUtils.saveMessageError(request, missatge("diposit.missatge.esborrar.no.sortidaDiposit", request));
                } else {
                	logger.info("Eliminant el lot [" + id + "]");
                	oliInfraestructuraEjb.setHibernateTemplate(getHibernateTemplate());
                	oliInfraestructuraEjb.lotEsborrar(Long.valueOf(id));
                }
            } catch (Exception ex) {
                logger.error("Error esborrant el lot [" + id + "]", ex);
//                exito = false;
            }
        }
