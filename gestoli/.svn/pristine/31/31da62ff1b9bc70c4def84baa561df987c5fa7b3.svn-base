...
public class QualitatPlaFormacioDelegate implements ApplicationContextAware {
	
	private static Logger logger = Logger.getLogger(QualitatPlaFormacioDelegate.class);
    ...
    public ModelAndView list(
        ...
        try {
        	...
        	Collection llistat = oliQualitatEjb.formacioCercaTotsPerEstabliment(idEstablecimiento);
        	oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
        	myModel.put("idEstablecimiento",idEstablecimiento);
        	myModel.put("llistat", llistat);
        	myModel.put("path", "qualitat_PlaFormacio");
            logger.info("Obtenint llistat de PlaFormacio: " + llistat.size() + " registres trobats");

        } catch (Exception ex) {
            logger.error("Error obtenint llistat de PlaFormacio", ex);
            ControllerUtils.saveMessageError(request, missatge("qualitat.PlaFormacio.missatge.llistat.no", request));
        }
        ...
        for (int i=0; i<ids.length; i++) {
            ...
            if (id != null && !"".equals(id)) {
                try {
                	...
                	logger.info("Eliminant el PlaFormacio [" + id + "]");
                	oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
                    oliQualitatEjb.formacioEsborrar(Long.valueOf(id));

                } catch (Exception ex) {
                    logger.error("Error esborrant el PlaFormacio [" + id + "]", ex);
                    exito = false;
                }
            }
        }
