 ...
 *
 */
public class ConsultaTrasabilitatOlivicultorLlistatController extends SimpleFormController {

    /*
     * Objeto para escribir mensajes de log.
     */
	private static Logger logger = Logger.getLogger(ConsultaTrasabilitatOlivicultorLlistatController.class);
    ...
    throws ServletException {
    	ConsultaTrasabilitatOlivicultorLlistatCommand consulta = (ConsultaTrasabilitatOlivicultorLlistatCommand) command;
    	...
    	try {
    		...
    		if (consulta.getIdOlivicultor() != null) myModel.put("idOlivicultor", consulta.getIdOlivicultor());
    		if (consulta.getIdEstabliment() != null) myModel.put("idEstabliment", consulta.getIdEstabliment());    		
    		myModel.put("path", "consultar_trasabilitatOlivicultor");
    		if (consulta.getFromEstabliment() != null && consulta.getFromEstabliment().equalsIgnoreCase("true")){	
	    		...
	    		if(request.getParameter("fromEstabliment")!= null){
	    			...
	    			if(establiment.getNom()!= null && !establiment.getNom().equalsIgnoreCase("")){
	    				myModel.put("path", "consulta_trasabilitatOlivicultorEstabliment");
...
//    		//Olivicultors de la llista
    		oliConsultaEjb.setHibernateTemplate(getHibernateTemplate());
    		Collection olivicultors = oliConsultaEjb.findOlivicultorsByEstabliment(idEstabliment);
    		myModel.put("olivicultors", olivicultors);
    		...
    		if (Util.isDataCorrecta(request.getParameter("dataInici"), "dd/MM/yyyy")
	    		...
	    		if(request.getParameter("idOlivicultor") != null && !request.getParameter("idOlivicultor").equals("")){
	    			Long idOlivicultor = Long.valueOf(request.getParameter("idOlivicultor"));
	    			consulta.setIdOlivicultor(idOlivicultor);
	    			llistat = oliConsultaEjb.trasabilitatOlivicultor(dataInici, dataFi, idOlivicultor, idEstabliment);
	    		} else {
	    			oliConsultaEjb.setHibernateTemplate(getHibernateTemplate());
	        		llistat = oliConsultaEjb.trasabilitatPartides(dataInici, dataFi, idEstabliment);
	    		}
    ...
    protected Object formBackingObject(HttpServletRequest request) throws ServletException {
    	ConsultaTrasabilitatOlivicultorLlistatCommand consulta = (ConsultaTrasabilitatOlivicultorLlistatCommand)request.getSession().getAttribute("consulta");
    	if (consulta == null) 
    		consulta = new ConsultaTrasabilitatOlivicultorLlistatCommand();
    	else 
    	...
    	try{
    		...
    		if(request.getParameter("idOlivicultor") != null && !request.getParameter("idOlivicultor").equals("")) {
    			consulta.setIdOlivicultor(Long.valueOf(request.getParameter("idOlivicultor")));
    		}
