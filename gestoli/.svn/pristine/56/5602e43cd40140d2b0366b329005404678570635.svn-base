...
public class GestioInfografiaDelegate implements ApplicationContextAware {
	
	private static Logger logger = Logger.getLogger(GestioInfografiaDelegate.class);
    ...
    public ModelAndView list(
        ...
        try {
        	...
        	Collection llistat = oliInfraestructuraEjb.gestioInfografiaCercaTots();
        	oliInfraestructuraEjb.setHibernateTemplate(getHibernateTemplate());
        	myModel.put("llistat", llistat);
        	myModel.put("path", "gestioInfografia");
            logger.info("Obtenint llistat de gestioInfografia: " + llistat.size() + " registres trobats");

        } catch (Exception ex) {
            logger.error("Error obtenint llistat de gestioInfografia", ex);
            ControllerUtils.saveMessageError(request, missatge("gestioInfografia.missatge.llistat.no", request));
        }
        ...
        for (int i=0; i<ids.length; i++) {
            ...
            if (id != null && !"".equals(id)) {
                try {
                	...
                	logger.info("Eliminant la gestioInfografia [" + id + "]");
                	oliInfraestructuraEjb.setHibernateTemplate(getHibernateTemplate());
                	oliInfraestructuraEjb.gestioInfografiaEsborrar(Long.valueOf(id));

                } catch (Exception ex) {
                    logger.error("Error esborrant la gestioInfografia [" + id + "]", ex);
                    exito = false;
                }
            }
        }
