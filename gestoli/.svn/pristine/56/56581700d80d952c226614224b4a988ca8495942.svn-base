...
public class QualitatSubministreDelegate implements ApplicationContextAware {
	
	private static Logger logger = Logger.getLogger(QualitatSubministreDelegate.class);
    ...
    public ModelAndView list(
        ...
        try {
    		...
    		Establiment est = (Establiment)session.getAttribute(establimentSessionKey);
	   		...
	   		Long idProveidor = new Long(Long.parseLong(request.getParameter("idProveidor")));
	    	Collection proveidors = oliQualitatEjb.proveidorsPerEstabliment(idEstabliment);
	    	myModel.put("proveidors", proveidors);
	    	
	    	oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
	    	if (request.getParameter("idProveidor")!=null && !request.getParameter("idProveidor").equals("")){
	    		llistat = oliQualitatEjb.subministrePerProveidor(idProveidor);
	    	}
	        ...
	        /*	if (oliQualitatEjb.existenZonasActiusEnEstabliment(idEstablecimiento).booleanValue()){
	        	...
	        	myModel.put("idEstablecimiento",idEstabliment);
	        	myModel.put("idProveidor",idProveidor);
	        	myModel.put("llistat", llistat);
	        	myModel.put("path", "qualitat_Subministres");
	            logger.info("Obtenint llistat de Subministres: " + llistat.size() + " registres trobats");

        	//}
	         
        } catch (Exception ex) {
            logger.error("Error obtenint llistat de subministres", ex);
            ControllerUtils.saveMessageError(request, missatge("qualitat.subministres.missatge.llistat.no", request));
        }
        ...
        for (int i=0; i<ids.length; i++) {
            ...
            if (id != null && !"".equals(id)) {
                try {
                	...
                	logger.info("Eliminant el Subministre [" + id + "]");
                	oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
                    oliQualitatEjb.subministreEsborrar(Long.valueOf(id));

                } catch (Exception ex) {
                    logger.error("Error esborrant el Subministre [" + id + "]", ex);
                    exito = false;
                }
            }
        }

        String forward = deleteView + "?codi=" + request.getParameter("idProveidor");
