		...
		if(trasllat.getEsDiposit()){
				...
				if (traza.getTipus().intValue() == Constants.CODI_TRAZA_TIPUS_SORTIDA_DIPOSIT){
					...
					if (sd != null){
						filaReport = new FilaReport(
								...
								(sd.getCategoriaOli() != null) ? sd.getPartidaOli().getNom():null);
					}
					break;
				}
			}
			...
			if (filaReport == null){
				...
				if (dip != null) {
					...
					if (dip.getPartidaOli() != null) partida = dip.getPartidaOli().getNom();
				}
				filaReport = new FilaReport(codDip, capacitat, acidesa, categoria, partida); 
			}
