/**
 * ProcesAcceptarOliGranelFormController.java
 */
 ...
 */
public class ProcesAcceptarOliGranelFormController extends SimpleFormController {

    private static Logger logger = Logger.getLogger(ProcesAcceptarOliGranelFormController.class);
    ...
    throws ServletException {
    	...
    	ProcesAcceptarOliGranelCommand acceptarOliGranelCommand = (ProcesAcceptarOliGranelCommand)command;    	
		...
		if (isCreate(request)) {
			try{ 
				...
				//Tenim 'n' dipòsits d'origen de l'establiment origen i fem el mateix que un trasbals al dipòsit destí.
				Double[] litresExtrets = new Double[acceptarOliGranelCommand.getDipositsOrigen().length];
				Double litrosAfegits = 0D;
				for(int i=0; i<litresExtrets.length; i++){
					Diposit dip = oliInfraestructuraEjb.dipositAmbId(acceptarOliGranelCommand.getDipositsOrigen()[i].getId());
					litresExtrets[i] = dip.getVolumTrasllat();
					litrosAfegits += dip.getVolumTrasllat();
				}
				
				Diposit dipOrigen = oliInfraestructuraEjb.dipositAmbId(acceptarOliGranelCommand.getDipositsOrigen()[0].getId());
				Diposit dipDesti = oliInfraestructuraEjb.dipositAmbId(acceptarOliGranelCommand.getDipositDesti().getId());
				oliProcessosEjb.acceptarTrasllatOli(
						acceptarOliGranelCommand.getData(),
						acceptarOliGranelCommand.getDipositsOrigen(), 
						acceptarOliGranelCommand.getDipositDesti(), 
						litresExtrets, 
						litrosAfegits, 
						dipDesti.getAcidesa(), 
						dipOrigen.getEstabliment(), 
						dipDesti.getEstabliment(),
						desqualificat, 
						pendentQualificar, 
						mescla,
						Long.parseLong((String)request.getSession().getAttribute("trasllatId")));
				
				//Llevam que els diposits s'estiguin traslladant
				Long idTrasllat = Long.parseLong((String)request.getSession().getAttribute("trasllatId"));
				oliInfraestructuraEjb.setHibernateTemplate(getHibernateTemplate());
				Trasllat t = oliInfraestructuraEjb.findTrasllatById(idTrasllat);
		        t.setTraslladant(false);
		        oliInfraestructuraEjb.setHibernateTemplate(getHibernateTemplate());
		        oliInfraestructuraEjb.trasllatModificar(t);
		        
				ControllerUtils.saveMessageInfo(request, missatge("proces.moureDiposit.missatge.ok"));
				
	    	} catch (Exception e) {
	    		 logger.error("Error processant el moviment de l'oli", e);
	             ControllerUtils.saveMessageError(request, missatge("proces.acceptarOliGranel.missatge.no"));
			}
		}
    ...
    protected Map referenceData(
        ...
        oliInfraestructuraEjb.setHibernateTemplate(getHibernateTemplate());
        if (destiList.size() != 0){ 
        	desti = (Diposit)(oliInfraestructuraEjb.dipositsInfo(destiList.toArray())).iterator().next();
        }
        ...
        Long idTrasllat = Long.parseLong((String)request.getSession().getAttribute("trasllatId"));
        Trasllat t = oliInfraestructuraEjb.findTrasllatById(idTrasllat);
        myModel.put("origens", t.getDiposits());
    	...
    	if(errors.hasGlobalErrors()){
    		myModel.put("missatgesError", missatge("error.capacitat.incorrecte2"));
    	}
    ...
    protected Object formBackingObject(HttpServletRequest request) throws ServletException {
    	ProcesAcceptarOliGranelCommand acceptarOliGranel = new ProcesAcceptarOliGranelCommand();
    	...
    	try {    		    		
			...
			Diposit[] dipositsOrigen = new Diposit[t.getDiposits().size()];
			
			for (int i = 0; i < dipositsOrigen.length; i++) {
				dipositsOrigen[i] = new Diposit();
			}
			
			acceptarOliGranel.setDipositsOrigen(dipositsOrigen);
			
			Diposit dipositDesti = new Diposit();
			acceptarOliGranel.setDipositDesti(dipositDesti);
			
		} catch (Exception e) {
			logger.error("EXCEPTION", e);
		}

        return acceptarOliGranel;
