 ...
 * 
 */
public class HistoricFincasDao {
	private static Logger logger = Logger.getLogger(HistoricFincasDao.class);
	...
	public Collection findAll() throws InfrastructureException {
		...
		try {
			...
			col = getHibernateTemplate().find("from HistoricFinca as finca order by finca.nom"); 
	...
	public Collection findAllOlivicultor(Long idOlivicultor) throws InfrastructureException {
		...
		try {
			...
			String q = "from HistoricFinca as finca where finca.olivicultor.id ="+idOlivicultor+" order by finca.nom";
	...
	public Collection findAllIdOlivicultor(Long idOlivicultor) throws InfrastructureException {
		...
		try {
			...
			String q = "select finca.id from HistoricFinca as finca where finca.olivicultor.id = "+idOlivicultor+" order by finca.id"; 
			col = getHibernateTemplate().find(q);
		} catch (HibernateException ex) {
	...
	public List findByOlivicultor(Long olivicultorId) throws InfrastructureException {
		...
		try {
			...
			String q = "select fin from HistoricFinca fin where fin.olivicultor.id = " +olivicultorId+ " order by fin.id";
	...
	public List findActiusByOlivicultor(Long olivicultorId) throws InfrastructureException {
		...
		try {
			...
			String q = "select fin from HistoricFinca fin where fin.olivicultor.id = " +olivicultorId+ " and fin.actiu = 'true' order by fin.id";
			establiments = getHibernateTemplate().find(q);
		} catch (HibernateException ex) {
	...
	public List fincasOlivicultorCercaTotsActiusConPlantacionesConDescomposicion(Long olivicultorId) throws InfrastructureException {
		...
		try {
			...
			String q = "select distinct pla.finca from HistoricPlantacio as pla" +
	...
	public Long findOlivicultorDeFinca(Long idFinca) throws InfrastructureException {
		...
		try {
			...
			String q = "select fin.olivicultor.id from HistoricFinca fin where fin.id = " +idFinca;
	...
	public boolean existenFincasAsociadasOlivicultores(Long idOlivicultor) throws InfrastructureException {
		...
		try {
			...
			String q = "select count(fin.id) from HistoricFinca fin " +
	...
	public Finca getById(Long id) throws InfrastructureException {
		...
		try {
			...
			finca = (Finca)getHibernateTemplate().load(HistoricFinca.class, id);
		} catch (HibernateException ex) {
	...
	public Long getFincaName(String nombreFinca, String idOlivicultor) throws InfrastructureException {
		...
		try {
			...
			String q = "select finca.id from HistoricFinca as finca where finca.nom = '"+nombreFinca+"' and finca.olivicultor.id = '"+idOlivicultor+"'";
			fincas = getHibernateTemplate().find(q);
			if (fincas.size() > 0) {
				logger.debug("getFincaName fin");
	...
	public Double getSuperfFincasDO(Long idTemporada, Double valorInicial, Double valorFinal) throws InfrastructureException {
		...
		try {
			...
			String q="select dpl.plantacio.finca.id, sum(dpl.superficie) from HistoricDescomposicioPlantacio dpl where dpl.plantacio.finca.olivicultor.campanya.id="+idTemporada;
			...
			if (fincasRango.size()>0){
				...
				q = "select sum(dpl.superficie) from HistoricDescomposicioPlantacio dpl";
	 ...
	 * @throws InfrastructureException
	 */
	public void makePersistent(HistoricFinca finca) throws InfrastructureException {
		try {
	 ...
	 * @throws InfrastructureException
	 */
	public void makePersistent(HistoricFinca finca, Session session) throws InfrastructureException {
		try {
			logger.debug("makePersistent ini");
	 ...
	 * @throws InfrastructureException
	 */
	public void makeTransient(HistoricFinca finca) throws InfrastructureException {
		try {
	...
	public List findFinquesDeBaixa() throws InfrastructureException {
		try {
			String q =	"from HistoricFinca f where f.deBaixa = true";
