...
*/
public class ProcesEntradaOlivaTaulaValidator implements Validator {

	private static Logger logger = Logger.getLogger(ProcesEntradaOlivaTaulaValidator.class);
	...
	public void validate(Object obj, Errors errors) {
		ProcesEntradaOlivaTaulaCommand procesEntradaOlivaTaulaCommand = (ProcesEntradaOlivaTaulaCommand)obj;
		...
		ValidationUtils.rejectIfEmpty(
				errors,
				"tipusOlivaTaula",
				"error.tipusOlivaTaula.buit",
				"El tipus d'oliva de taula no pot estar buit");
		...
		if ((procesEntradaOlivaTaulaCommand.getQuantitat() != null) && (procesEntradaOlivaTaulaCommand.getQuantitat().doubleValue() < 0)) {
			errors.rejectValue("quantitat", "error.quantitat.buit" ,"El camp Quantitat total d'oliva ha de ser major que zero");
		}

		boolean esHora = true;
		if ((procesEntradaOlivaTaulaCommand.getHora() != null) && (!procesEntradaOlivaTaulaCommand.getHora().equalsIgnoreCase(""))) {
			esHora = Util.isHora(procesEntradaOlivaTaulaCommand.getHora());
		}
		if (!esHora) {
		...
		if ((procesEntradaOlivaTaulaCommand.getQuantitat() != null) && (procesEntradaOlivaTaulaCommand.getQuantitat().doubleValue() > 0)) {
			try {
				...
				if (!procesEntradaOlivaTaulaCommand.getIdsPlantacions().equals("")) {
					Double sumaTotal = 0.0;
					Integer x = 0;
					String[] plants = procesEntradaOlivaTaulaCommand.getIdsPlantacions().split(",");
					
					VarietatOlivaCommand[] variedades = procesEntradaOlivaTaulaCommand.getVariedades();
...
//							x++;
					...
					Double sumCommand = Double.parseDouble(df.format(procesEntradaOlivaTaulaCommand.getQuantitat()).replaceAll(",", "."));
					if ((sumCommand.doubleValue() - sumaTotal.doubleValue())< 0.0000) {
						if (procesEntradaOlivaTaulaCommand.getQuantitat().doubleValue() < 0) {
							errors.rejectValue("quantitat", "error.quantitat.descNegativa" ,"El valor dels Kg. de les varietats no pot ser negatiu ");
						} else {
				...
				oliInfraestructuraEjb.setHibernateTemplate(getHibernateTemplate()); 
				if(!oliInfraestructuraEjb.esDataCampanyaCorrecte(procesEntradaOlivaTaulaCommand.getDataExecucio())){
					correcte = false;
				}
				...
				logger.error("Error validant l'entrada d'oli. ProcesEntradaOlivaTaulaValidator", e);
			}
		}
	}
