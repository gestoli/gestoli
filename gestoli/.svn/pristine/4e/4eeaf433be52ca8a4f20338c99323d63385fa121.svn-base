...
public class QualitatPlaNetejaRealitzacioDelegate  implements ApplicationContextAware {
	
	private static Logger logger = Logger.getLogger(QualitatPlaNetejaRealitzacioDelegate.class);
     ...
     * 
     * @see org.springframework.web.servlet.mvc.Realitzacioler#handleRequest(javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse)
     */
    public ModelAndView list(
        ...
        try {
			...
			ResourceBundle bundle = ResourceBundle.getBundle("messages",new java.util.Locale(lang));
	   		Collection llistat = oliQualitatEjb.netejaRealitzacioPerPlaNeteja(idNeteja);
            ...
            logger.info("Obtenint llistat de PlaNetejaRealitzacio: " + llistat.size() + " registres trobats");

        } catch (Exception ex) {
            logger.error("Error obtenint llistat de PlaNetejaRealitzacio", ex);
        ...
        for (int i=0; i<ids.length; i++) {
            ...
            if (id != null && !"".equals(id)) {
                try {
                	...
                	logger.info("Eliminant el PlaNetejaRealitzacio [" + id + "]");
                	oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
                    oliQualitatEjb.netejaRealitzacioEsborrar(Long.valueOf(id));

                } catch (Exception ex) {
                    logger.error("Error esborrant el PlaNetejaRealitzacio [" + id + "]", ex);
                    exito = false;
                }
            }
        }
