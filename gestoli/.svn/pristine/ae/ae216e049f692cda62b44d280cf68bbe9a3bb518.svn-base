...
public class QualitatProveidorRGSADelegate implements ApplicationContextAware {
	
	private static Logger logger = Logger.getLogger(QualitatProveidorRGSADelegate.class);
        ...
        for (int i=0; i<ids.length; i++) {
            ...
            if (id != null && !"".equals(id)) {
                try {
                	...
                	logger.info("Eliminant el RGSA [" + id + "]");
                	oliQualitatEjb.setHibernateTemplate(getHibernateTemplate());
                    oliQualitatEjb.proveidorRGSAEsborrar(Long.valueOf(id));

                } catch (Exception ex) {
                    logger.error("Error esborrant el RGSA [" + id + "]", ex);
                    exito = false;
                }
            }
        }
