...
<title><fmt:message key="consulta.declaracioMensual.agenciaOli.title" /></title>
<%--
...
<script type="text/javascript" src="js/form.js"></script>
	...
	<script type="text/javascript">
		$(function(){
		  ...
		  $("#enlaceAceptarForm").click(function(){
			  openDialog();
		  });
...
<script type="text/javascript">
function openDialog() {
    var controllerUrl = '<%=request.getContextPath()%>/GenerarPdf.html';
     $('#loading')
           .html('<div class="loading"><div>Calculant declaració nacional</div></div>')
           .load(controllerUrl)
           .dialog({
                 title: 'Declaració nacional',
                 modal: true,
                 width: 'auto'
            });
}
...
<c:if test="${(not empty esDoGestor || not empty esAdministracio || not empty esDoControlador ) && empty idEstabliment }">
	...
	<display:table name="llistat" requestURI="" id="establiment" pagesize="20" sort="list" cellpadding="0" cellspacing="0" class="listadoEstrecho selectable">
		<display:column titleKey="establiment.camp.nom" sortable="true" sortProperty="nom">
			<a href="AgenciaOliDeclaracioMensual.html?idEst=<c:out value="${establiment.id}"/>&amp;fromEstabliment=true"><c:out value="${establiment.nom}" /></a>
		</display:column>
...
<c:if test="${not empty idEstabliment }">
	...
	<form id="formulario" action="<%=request.getContextPath()%>/GenerarPdf.html" method="post" class="extended seguit">
		...
		<input type="hidden" id="tipus" name="tipus" value="16" /> 
	...
	<div class="separadorH"></div>
	<div id="loading" style="display:none;margin-left: 300px">
<!-- 		<img src='img/ajax-loader.gif' alt='Calculant declaració nacional'/> -->
	</div>
</c:if>
